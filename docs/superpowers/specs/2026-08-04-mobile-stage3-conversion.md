...
| 5 🚧 | **주문 내역 폰 전용 화면** — 사장님 시안 확정(2026-08-04): **1-C**(2줄 압축 줄+오른쪽 금액열·tabular-nums·상태 배지 앞) + **2-A**(위쪽 알약 칩 4개, 개수 배지 — 목록·송장·CS·마진) + **3-C**(요약 KPI 3칸=신규·오늘매출·품절위험은 **주문 화면 상단에만**, 홈에는 「주문 보러 가기」 바로가기만 — 같은 숫자 두 곳 금지). 시안 파일=바탕화면 「모음전 폰 주문화면 시안 v1.html」, **시안=코드**(그 배치 그대로 구현) | 괴물 | 폰 전용 신축(/mobile/orders) — retrofit 아님. 기존 주문 API 재사용, 새 집계 발명 금지 |
...
| 항목 | 확정 | 내용 |
|---|---|---|
| A 송장 | **A-1 + A-4 합침** | 위 = 현황 3칸(발송대기·오늘 발송·미매칭) · 아래 = 대기 목록, 줄 누르면 그 자리에서 택배사+송장번호 입력·저장 |
| B CS | **B-2** | 유형 칩(전체·취소·반품·교환·문의)로 거르는 목록 — cs/claims.json + cs/inquiries.json 배선 |
| C 마진 칩 | **C-4** | 기간 칩(오늘/7일/이번달) + 숫자 6칸(매출·마진·마진율·주문·취소·적자) |
| D 매트릭스 | **D-1** | 검색 → 옵션 카드(소싱처별 가격·재고). 폰 화면 신축 |
| E 마진 계산기 | **E-1** | 기간 요약(정산 예정/완료 + 마켓별 막대). 폰 화면 신축 |
| F 크롤가이드 | **F-2** | 읽기용 목차 → 문서 읽기 화면 |
