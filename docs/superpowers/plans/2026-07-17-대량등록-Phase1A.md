...
def test_second_account_does_not_overwrite_first(session, monkeypatch):
    """★ 같은 마켓의 다른 계정에 올려도 앞 계정의 상품ID 가 사라지지 않는다.

    설계서 §7-13 「동일 계정 재등록 차단 · 타 계정은 별도 설정으로 허용」.
    이게 깨지면 Phase 2 가격·재고 자동갱신이 엉뚱한 계정으로 나간다 (금전 손실).
    """
    monkeypatch.setenv('LIVE_REGISTER_ARMED', '1')
    d = _draft(session)
    register_draft(session, d.id, 'smartstore', category_code='1',
                   account_key='acctA', _send=lambda m, b: {'originProductNo': 111})
    register_draft(session, d.id, 'smartstore', category_code='1',
                   account_key='acctB', _send=lambda m, b: {'originProductNo': 222})

    rows = {r.account_key: r for r in
            session.query(ProductDraftMarket).filter_by(draft_id=d.id).all()}
    assert set(rows) == {'acctA', 'acctB'}, '계정별로 행이 남아야 한다'
    assert rows['acctA'].market_product_id == '111', 'A 계정 상품ID 가 덮어써졌다'
    assert rows['acctB'].market_product_id == '222'
```
