...
| 6 | ✅ **스스 이미지 업로드 배선 완료 (IU-1~3, 2026-07-18) — 실호출만 미검증** | 폼 공개URL → `image_prep.prepare_cdn_images`(fetch→`upload_images`) → `cdn_images_json` → 재컴파일 배선됨. 게이트 뒤 전용(arm 없이 라이브 없음). **실호출은 미검증**: 실제 소싱처 URL fetch·네이버 CDN 업로드·옵션+다중이미지 조합을 실계정 초안등록으로 확인해야 함. `compile_smartstore(require_cdn_images=False)` 가 게이트 앞 예비컴파일(이미지 없이 A/S·옵션 오류만) 담당 |
