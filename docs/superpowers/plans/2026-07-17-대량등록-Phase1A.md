...
> ⚠️ **payload 는 새로 짜지 말고 라이브 검증본 `lemouton/registration/coupang.py::_build_payload`(2026-04-22 실증)와 필드 대조할 것.** 내 초안이 이걸 안 해서 쿠팡 `createProduct` 필수(required:O) 필드 5개(freeShipOverAmount·deliveryChargeOnReturn·unionDeliveryType·vendorUserId·item.maximumBuyForPersonPeriod)를 빠뜨렸다 — 순수함수 테스트로는 절대 못 잡고 라이브에서 400. 또 item 숫자필드는 검증본이 **문자열**('0'·'1'·'false')로 보낸다(우리 초안은 int/bool = 타입 불일치). ★교훈: 마켓 payload 는 지도(marketplace_api_map.json)의 required 표 + 라이브 검증본, 둘 다 대조. vendor 계약에 vendor_user_id·return_charge_name 추가됨.
...
def create_draft():
    ...
    # ★ 숫자칸은 전부 coerce_int 로 검증한다 — bare int() 는 '15,000'·'abc' 에 500 을 낸다.
    #   sale_price 만 막고 나머지(정가·재고·배송비·반품비)를 안 막으면 그것들이 500 을 낸다.
    try:
        sale_price = coerce_int(p.get('sale_price'), '판매가') or 0
        normal_price = coerce_int(p.get('normal_price'), '정상가')  # None 이면 미입력
        stock_quantity = coerce_int(p.get('stock_quantity'), '재고') or 0
        delivery_fee = (coerce_int(p['delivery_fee'], '배송비')
                        if p.get('delivery_fee') not in (None, '') else 3000)
        return_fee = (coerce_int(p['return_fee'], '반품비')
    ...
    try:
        d = ProductDraft(
            ...
            # 배송비 0 = '무료배송' 이라는 뜻 있는 값이라 '미입력'(→기본 3000)과 구분했다.
            #   (위에서 coerce_int 로 이미 검증·기본값 처리 완료)
...
- Test: `프로그램/_시스템/tests/registration/test_drafts_route.py` (★ Task 8 이관 — 라우트 자동 테스트)
...
>
> ★ **Task 8 에서 이관된 라우트 테스트 의무** (drafts.py 라우트는 지금까지 자동 테스트가 없다 — 수기화면이 처음 라우트를 실제로 두드리는 지점이라 여기서 함께 덮는다). `test_drafts_route.py` 에 `monkeypatch.setenv('DISABLE_AUTH','1')` + `create_app()` 픽스처(관례: `tests/registration/test_bulk_mode.py`)로:
> - `POST /bulk/api/drafts` — 정상 200+id / 이름없음 400 / 잘못된 옵션(`stock:"빨강"`) 400 / **잘못된 숫자(`normal_price:"abc"`·`delivery_fee:"1,000"`) 400**(500 아님 — Task 8 코드리뷰가 라우트 int() 500 지적, 플랜은 coerce_int 로 고쳐뒀으니 이 테스트가 그걸 고정)
> - `POST …/register/<market>` 게이트 미설정 → `{'ok':False,'blocked':True}` **200**(500 아님) / 잘못된 market(`11st`) 400 / 없는 draft id 404 / 카테고리 없음 400
> - `GET /bulk/api/drafts` → 만든 draft 반환, market 행마다 `account_key`
> ⚠️ **`init_db()` fresh-SQLite 관찰**(Task 8 스펙리뷰): fresh SQLite 에서 `create_app()` 의 `init_db()`(app.py:102)가 route-time 모델 import(app.py:203) 전에 돌아 `NoReferencedTableError(market_accounts)` 가능. 라이브(Supabase)는 테이블이 이미 있어 안 걸린다. **표준 `create_app()` 픽스처로 라우트 테스트를 돌려 이게 실제로 터지는지 확인** — 터지면 부트스트랩 순서 버그를 Task 9 UI 가 의존하기 전에 잡는 것이고, 안 터지면 Supabase 전용임을 확정한다.
...
## ⚠️ LIVE 실전송 전 확인 항목 (게이트 켜기 전 필수)

순수함수 테스트로는 라이브 API 수용 여부를 증명 못 한다. LIVE_REGISTER_ARMED=1 로 처음
등록하기 전에 아래를 **실계정 초안 등록으로 실증**한다(4대 마켓 실전송 검증 때와 같은 방식):

| # | 항목 | 왜 미실증인가 |
|---|---|---|
| 1 | **쿠팡 필수필드 전수** | 지도 create-product `[필수]` 최상위 23·item 16 전부 payload 에 있음(프로그램 대조 확인). 라이브 400 위험은 낮지만 실증은 안 됨 |
| 2 | **쿠팡 `contentsType: 'HTML'`** | 우리 content 는 실제 HTML → HTML 라벨이 정직. 라이브 검증본은 `TEXT`(HTML을 TEXT로 = 표시버그 의심). **HTML 로 보냈을 때 상세가 제대로 렌더되는지** 확인 |
| 3 | **쿠팡 `outboundShippingTimeDay: '3'`** | 설계서 §7-10 기본 3일. 검증본은 '1'(그 계정값). 값 차이는 400 아님, 업무 선택 |
| 4 | **쿠팡 무옵션(flat) `attributes` = `색상:단일`** | 임시 placeholder. 쿠팡 attributes 는 카테고리 바인딩이라 임의 타입명이 통과할지 미검증. **무옵션 단일상품 쿠팡 등록이 Phase 1A 지원 대상인지도 미확정** |
| 5 | **스스 옵션·다중이미지 조합** | 스스 라이브 검증본(create_product.py)은 무옵션·단일이미지였음. optionCombinations·optionalImages 조합은 미실증 |
| 6 | 🔴 **스스 이미지 업로드 배선 자체가 안 됨 (미검증 아님 — 미배선)** | 폼 이미지는 `images`→`images_json`(쿠팡 공개URL 칸)으로만 흐른다. `bulk_manual.js` 가 `cdn_images` 를 안 보내고 `upload_images()` 는 호출자가 없다. 그래서 LIVE 켜고 스스 등록하면 `compile_smartstore` 가 **CDN 이미지 없음으로 100% 컴파일 실패**한다(게이트 뒤라 안전하지만 실사용 불가). ★**LIVE 켜기 전 반드시**: 폼 이미지(바이트/URL)→`upload_images`→`cdn_images_json` 배선. Phase 1A 는 '게이트 뒤 코드 완성' 이지 '실사용 가능' 이 아니다 |
| 7 | **쿠팡 UI 트리거·vendor 선택 없음** | 등록 버튼이 `bulk_manual.js:98` 에서 smartstore 하드코딩. 쿠팡은 컴파일러만 있고 UI 진입점·vendor dict 수집이 없다. Phase 2 계정선택과 함께 |
