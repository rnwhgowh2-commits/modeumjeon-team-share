...
def fetch_via_playwright(product_url: str, headless: bool = True) -> CrawlResult:
...
def smoke(url: str, label: str = ""):
    ...
    try:
        ...
        if r.options:
            o = r.options[0]
            print(f"     sale={o.get('sale_price'):,}원  stock={o.get('stock')}")
            ...
            if 'card_benefit_price' in o:
                print(f"     카드혜택가={o.get('card_benefit_price'):,}원")
            if 'product_coupon_rate' in o:
                print(f"     상품쿠폰: rate={o.get('product_coupon_rate')} min_order={o.get('product_coupon_min_order')}")
