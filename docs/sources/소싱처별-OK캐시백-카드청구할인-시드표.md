...
## 4. ⚠️ 지금 이 시드는 가격을 1원도 바꾸지 않는다 (중요)

`compute_breakdown` 실측 결과 (표면가 100,000원):

| 소싱처 | 시드 전 최종매입가 | 시드 후 최종매입가 | OK캐시백 |
|---|---|---|---|
| 롯데온 | 97,200 | **97,200 (동일)** | 비활성 (택1 패배) |
| SSG | 97,200 | **97,200 (동일)** | 비활성 (택1 패배) |

**이유** — 카드 청구할인 행이 하나도 없으면 엔진은 legacy 경로로 돈다. legacy 의
`_is_payment()` 는 이름에 '캐시백' 이 들어가면 **결제수단으로 보고 택1 그룹**에
넣는다. 그래서 OK캐시백(1.1% / 2%)이 현대카드 2.73% 플로어와 경쟁해서 **진다**.

카드 청구할인 행이 1건이라도 들어오면 그 소싱처는 tagged 경로로 넘어가고,
그 순간 캐시백이 결제 택1에서 빠져나와 카드와 **함께** 차감된다
(`card_candidates._is_cashback`). 실측(가정값 삼성카드 7% 주입 시 롯데온):

```
삼성셀렉트 적립 1%      -1,000 → 99,000
삼성카드 청구할인 7%    -6,930 → 92,070
OK캐시백 1.1%           -1,012 → 91,058   최종매입가 91,000
```

> 즉 **OK캐시백을 실제로 매입가에 반영하려면 §3 의 카드 청구할인 표를 채워야 한다.**
> 시드만으로는 데이터가 준비될 뿐 가격은 움직이지 않는다(= 배포 blast radius 0).
