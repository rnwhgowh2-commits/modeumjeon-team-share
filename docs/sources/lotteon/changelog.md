...
### 라이브 검증 결과 (2026-05-17) — 사용자 제공 2 URL ✅
1. **LO2158462914** (르무통 메이트 메리노울 운동화) — sale 126,060원, 117옵션
   - 자동 적용: 스토어 즉시할인 6% / 롯데ON 즉시할인 10%
   - 추가 가능 (미적용): 첫구매 5천원 / 스토어찜 감사 쿠폰 — 조건 명시 정상 ✅
2. **PD46925922** (나이키 에어포스 1 07) — sale 149,000원, 13옵션
   - 5개 카드 할인 분리 추출 (롯데카드/토스/카카오/삼성 — 조건 70,000원 이상)
   - 조건부 적용 룰 정상 동작 ✅

⚠️ 양쪽 stock=0 — 옵션 mapping API 재고 필드 매핑 변경 가능성 (별도 진단 필요)
