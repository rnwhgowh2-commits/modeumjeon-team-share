...
1. `git checkout main && git pull origin main` — 최신화 (세션 시작 시 hook으로 이미 됐을 수 있으나 작업 착수 직전 한 번 더 확인)
2. 이슈 확인: `gh issue list`로 관련 이슈가 있는지 확인. 없고 작업이 이슈로 남길 만하면 `gh issue create --title "..." --body "..."`로 생성
3. 브랜치 생성 (origin/main 기준): `git checkout -b issue-<번호>-<짧은설명>` (예: `issue-42-price-calc-fix`)
   - 다른 작업과 병행해야 하면(예: add-source 스킬이 동시에 다른 워크트리에서 돈다) 별도 워크트리 사용: `git worktree add ../<repo>-issue-<번호> issue-<번호>-<설명>`
4. 보고 예: "이슈 #42 브랜치(`issue-42-price-calc-fix`) 생성, 여기서 작업 시작합니다."
...
5. **정리**: `git checkout main && git pull origin main` (로컬 main 최신화) + 사용한 워크트리가 있으면 `git worktree remove <경로>`
