...
DED_LO = {
    "블랙":"LO2158462485","다크네이비":"LO2158462341","그레이":"LO2158462407",
...
# (color, size, ded_price, ded_stock, hub_price, hub_stock)  — 라이브 omi 수집값(2026-06-19)
ROWS = [
...
("그레이","250",116910,20,116910,10),("그레이","255",161650,"재고있음(999)",116910,"품절"),("그레이","260",116910,"품절",116910,"품절"),
...
("오렌지","220",116910,5,116910,10),("오렌지","225",116910,"품절",116910,"품절"),("오렌지","230",161650,"재고있음(999)",116910,"품절"),
("오렌지","235",161650,"재고있음(999)",116910,"품절"),("오렌지","240",116910,10,116910,"품절"),("오렌지","245",116910,10,116910,"품절"),
...
("라이트블루","235",116910,38,116910,10),("라이트블루","240",116910,39,116910,10),("라이트블루","245",116910,40,116910,10),
...
wb = Workbook(); ws = wb.active; ws.title = "롯데온 재고·가격"
F = "맑은 고딕"
thin = Side(style="thin", color="D0D0D0")
border = Border(left=thin, right=thin, top=thin, bottom=thin)
hdr_fill = PatternFill("solid", fgColor="1F4E78")
ded_fill = PatternFill("solid", fgColor="EAF1FB")
hub_fill = PatternFill("solid", fgColor="FCEFE6")
out_fill = PatternFill("solid", fgColor="FDE2E2")
out_font = Font(name=F, color="C00000", bold=True)
link_font = Font(name=F, color="0563C1", underline="single", size=9)

# Title + note
ws["A1"] = "롯데온 색상별·사이즈별 재고·가격 (라이브 수집값 — 2026-06-19)"
ws["A1"].font = Font(name=F, bold=True, size=14)
ws.merge_cells("A1:H1")
ws["A2"] = "검증용: 각 URL 클릭 → 실제 롯데온 값과 단품/모음전 재고·가격 대조. ※모음전(허브)=다색 통합 리스팅, 단품=색상별 전용 리스팅 (재고풀이 다름). '재고있음(999)'=수량 미상 충분."
ws["A2"].font = Font(name=F, size=9, italic=True, color="555555")
...
for (color, size, dp, ds, hp, hs) in ROWS:
    ...
    if ded_lo:
        c3.value = "열기"; c3.hyperlink = url(ded_lo); c3.font = link_font
    else:
        c3.value = "(전용 없음)"; c3.font = Font(name=F, size=9, color="999999")
    ...
    if hp is None and hs is None:
        c6.value = "(사이즈 없음)"; c6.hyperlink = None; c6.font = Font(name=F, size=9, color="999999")
    ...
    # 품절 강조
    if ds == "품절":
        c5.font = out_font; c5.fill = out_fill
    if hs == "품절":
        c8.font = out_font; c8.fill = out_fill
    # 가격 포맷
    for pc in (4,7):
...
widths = {"A":11,"B":8,"C":11,"D":12,"E":15,"F":13,"G":13,"H":15}
for col, w in widths.items(): ws.column_dimensions[col].width = w
ws.freeze_panes = "A5"
ws.row_dimensions[1].height = 22

out = r"C:\dev\모음전 프로젝트\롯데온_색상사이즈_재고가격_검증_20260619.xlsx"
wb.save(out)
