...
async function _listingCollectIds(sel, attr, reSrc, moreSel, emptyText) {
  const seen = new Set();
  let re;
  ...
  let capped = false;
  if (moreSel) {
    try { capped = document.querySelectorAll(moreSel).length > 0; } catch (e) {}
  }
    ...
    for (const job of jobs) {
      ...
      const rule = (job.sel && job.attr && job.id_re)
        ? { sel: job.sel, attr: job.attr, id_re: job.id_re,
            more_sel: job.more_sel, empty_text: job.empty_text } : null;
