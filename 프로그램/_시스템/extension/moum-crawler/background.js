...
async function settleTick() {
  ...
  if (_settleRunning) {
    ...
    try {
      ...
      await settleSave(Object.assign({}, cur, { runStartedAt: 0, last: {
        at: Date.now(), ok: 0, verify: 0, fail: 0, orders: 0, soRows: 0, soFail: 0,
...
try {
  settleLoad().then(async (st) => {
    ...
    await settleSave(Object.assign({}, st, { runStartedAt: 0, last: {
      at: Date.now(), ok: 0, verify: 0, fail: 0, orders: 0, soRows: 0, soFail: 0,
