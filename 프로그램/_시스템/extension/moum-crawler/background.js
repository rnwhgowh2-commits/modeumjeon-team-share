...
// ══════════════════════════════════════════════════════════════════════════
//  [2026-08-19] 자가 새로고침 — 사장님이 매번 chrome://extensions 에서 ↻ 를 눌러야
//   git 으로 받은 새 코드가 적용되던 부담을 없앤다. 1시간마다 알람이 울려 이 파일의
//   4개 busy 플래그(_listingBusy·_loneBusy·_notionShotBusy·_settleRunning)가 **전부
//   false(유휴)일 때만** chrome.runtime.reload() 한다. 하나라도 바쁘면 건너뛰고 다음
//   시간을 기다린다 — 밀어붙이지 않는다(크롤 도중 reload 하면 열려 있던 창·탭이 반쯤
//   끊긴 채 남는다). ★첫 작성 때 _settleRunning 을 빠뜨렸었다 — 정산 회차는 계정별
//   로그아웃→로그인→수집을 최대 25분(_SETTLE_RUN_BUDGET_MS) 동안 도는데, 이 셋만 보면
//   그 도중에도 reload 가 끼어들어 로그인 절차가 반쯤 끊길 수 있었다(리뷰에서 잡힘).
//   ★버전 비교·업데이트 확인 없이 그냥 매시 시도하는 단순 설계가 의도다 — 유휴 중이면
//     디스크 코드가 안 바뀌었어도 reload 자체는 무해하다(굳이 비교 로직을 더할 이유 없음).
//   ⚠️아직 라이브 미검증 — 확장이 이 PC 브라우저에 로드되기 전에 작성한 코드다.
// ══════════════════════════════════════════════════════════════════════════
const MOUM_SELFUPDATE_ALARM = "moum-self-update";

function moumSelfUpdateCheckOnce() {
  if (_listingBusy || _loneBusy || _notionShotBusy || _settleRunning) {
    console.log("[moum-self-update] 바쁨 — 건너뜀(다음 시간에 재시도)", {
      listing: _listingBusy, lone: _loneBusy, notionShot: _notionShotBusy, settle: _settleRunning,
    });
    return;
  }
  console.log("[moum-self-update] 유휴 — 자가 새로고침 실행");
  try { chrome.runtime.reload(); } catch (_) {}
}
try {
  chrome.alarms.onAlarm.addListener((a) => {
    if (a && a.name === MOUM_SELFUPDATE_ALARM) moumSelfUpdateCheckOnce();
  });
} catch (_) {}
// ★alarms.create 는 같은 이름이 있으면 취소·대체한다 — SW 가 다른 알람·메시지로 깰
//   때마다 이 줄이 매번 돌면 카운트다운이 계속 리셋돼 영영 안 터질 수 있다(moum-auto-poll
//   과 같은 처방). → 없을 때만 만든다.
try {
  chrome.alarms.get(MOUM_SELFUPDATE_ALARM, (a) => {
    if (!a) { try { chrome.alarms.create(MOUM_SELFUPDATE_ALARM, { periodInMinutes: 60 }); } catch (_) {} }
  });
} catch (_) {}
