...
// 페이지 안에서 실행 — 상품 링크의 **번호만** 긁는다.
//   ★ 서버 `lemouton/sources/listing_discover.py` 와 **같은 규칙**이다. 번호만 보내고
//     주소 조립은 서버가 한다 — 광고 추적 꼬리표(?srsltid=)가 붙은 채 저장되면 같은
//     상품이 두 벌로 갈린다.
function _listingCollectIds() {
  const out = [];
  const seen = new Set();
  document.querySelectorAll('a[href*="/products/"]').forEach((a) => {
    const m = String(a.href || "").match(/\/products\/(\d+)/);
    if (m && !seen.has(m[1])) { seen.add(m[1]); out.push(m[1]); }
  });
  return out;
}

async function _listingScanOnePage(url) {
...
async function moumListingPollOnce() {
  ...
  try {
    for (const job of jobs) {
      const ids = new Set();
      let err = job.error || "";
      for (const pageUrl of (job.page_urls || [])) {
        try {
          for (const id of await _listingScanOnePage(pageUrl)) {
            ids.add(id);
