...
//   ★ [2026-08-08] **화면을 내려 가며** 모은다. 롯데온·롯데아이몰·SSF 검색은
//     페이지 넘김 파라미터가 없는 무한 스크롤이라, 안 내리면 첫 화면분만 걷힌다
//     (실측: 롯데온 48 · 아이몰 26 · SSF 27). 몇 번 내릴지는 **서버가 정한다**.
//   🔴 상한에 걸려 다 못 봤으면 `capped: true` 로 **말한다.** 조용히 자르면
//     사장님이 「이 검색엔 48개뿐」이라고 믿는다 — 없는 걸 없다고 믿게 만드는 짓이다.
async function _listingCollectIds(sel, attr, reSrc, rounds) {
  const seen = new Set();
  let re;
  ...
  sweep();                                   // 내리기 전 화면부터
  let capped = false;
  const max = Math.max(0, Number(rounds) || 0);
  let quiet = 0;                             // 내렸는데 안 늘어난 횟수
  for (let i = 0; i < max; i++) {
    ...
    sweep();
    if (seen.size === before) {
      // 두 번 연속 안 늘면 바닥이다 — 더 내려도 시간만 쓴다.
      if (++quiet >= 2) { capped = false; break; }
    } else {
      quiet = 0;
      // 마지막 바퀴까지 계속 늘고 있었다면 **아직 더 있는 것**이다.
...
async function _listingScanOnePage(url, rule) {
  ...
  try {
    ...
    // 🔴 주입에 **따로 시한**을 둔다. 페이지 로드 시한(위)은 스크롤 도는 동안을
    //   안 덮는다 — 여기가 무한대기가 되면 필터 하나가 폴링을 통째로 잡는다.
    //   스크롤 한 바퀴 1.2초 + 여유.
    const budget = 8000 + (Number(rule.scroll_rounds) || 0) * 2500;
    const out = await Promise.race([
      chrome.scripting.executeScript({
        target: { tabId }, func: _listingCollectIds,
        args: [rule.sel, rule.attr, rule.id_re, Number(rule.scroll_rounds) || 0],
      }),
      new Promise((_r, rej) => setTimeout(
        () => rej(new Error("훑는 중 시간 초과")), budget)),
    ]);
    const got = (out && out[0] && out[0].result) || null;
    ...
    for (const job of jobs) {
      ...
      const rule = (job.sel && job.attr && job.id_re)
        ? { sel: job.sel, attr: job.attr, id_re: job.id_re,
            scroll_rounds: job.scroll_rounds } : null;
