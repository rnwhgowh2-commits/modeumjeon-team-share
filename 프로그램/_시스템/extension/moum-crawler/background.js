...
async function _listingCollectIds(sel, attr, reSrc, moreSel, emptyText, clickPages, nextUrlRe) {
  const seen = new Set();
  let re;
  ...
  if (nextUrlRe) {
    ...
    try {
      ...
      if (m0 && m0[1]) url = m0[1].replace(/\\u0026/g, '&').replace(/\\\\//g, '/');
    } catch (e) {}
    ...
    for (let i = 1; i < want && url; i++) {
      try {
        ...
        url = (m2 && m2[1]) ? m2[1].replace(/\\u0026/g, '&').replace(/\\\\//g, '/') : null;
        if (!n) break;                              // 안 늘면 그만 (헛돌기 방지)
...
async function _listingScanOnePage(url, rule) {
  ...
  try {
    ...
    const budget = 20000 + Math.max(0, (Number(rule.click_pages) || 1) - 1) * 4000;
    ...
    for (const job of jobs) {
      ...
      const rule = (job.sel && job.attr && job.id_re)
        ? { sel: job.sel, attr: job.attr, id_re: job.id_re,
            ...
            click_pages: job.click_pages, next_url_re: job.next_url_re } : null;
