...
def test_cp_estimate_settle_formula():
    ...
    🔴 [2026-08-13 정정] 옛 시험은 `round(103000 * 0.8845)` 로 **구현식을 그대로
      ...
      그래서 여기선 **숫자를 손으로 적는다** — 구현이 바뀌면 여기서 걸리게.
        ...
        103,000 × 11.55% = 11,896.5 → 11,897 → 정산 91,103
        200,000 × 11.55% = 23,100.0 → 23,100 → 정산 176,900
    """
    assert oe._cp_estimate_settle(100000, 1, 3000) == 91103
...
def test_스스_M열은_상품정산만_배송비정산을_안_섞는다():
    ...
    규약: M열(`정산예정금액`) = **상품분만**. M 에 배송비 정산을 더하면 `_finalize_rows`
    가 거기에 배송비를 **또** 더해 N 이 부풀어 오른다.
      라이브 실측 1건: 상품 58,430 + 배송비정산 2,910 = M 61,340 → N 64,340 (2,910원 과다)
    같은 원인으로 수수료율도 1.32% 로 찍혔다(바른 값 6.00%).

    ★ [2026-08-12 갱신] N열 규약이 바뀌었다 — 더하는 값이 **고객배송비 전액이 아니라
      배송비 정산 실값**이다. 옛 규약(`N = M + 고객배송비`)은 사장님이 준 쿠팡 정산 엑셀
      실물 대조에서 뒤집혔다: **배송료 124건 전부** `4,000 → 수수료 132(3.30%) → 3,868`.
      마켓은 배송비에서도 수수료를 뗀다 — 전액을 더하면 「받을 돈」이 상시 과대다.
      이 시험이 지키는 것은 그대로다: **M 은 여전히 상품분만**, 배송비는 **한 번만**.
      달라진 건 「얼마를 더하는가」뿐이다.
    """
    since = dt.datetime(2026, 7, 5, tzinfo=KST)
    until = dt.datetime(2026, 7, 5, 23, tzinfo=KST)
    ...
    assert r["정산예정금액"] == 58430, (
        ...
        % r["정산예정금액"])
    assert r.get("_ship_settle") == 2910, (
        "배송비 정산 실값을 안 실어 보냈다 — N 이 고객배송비 전액으로 되돌아간다: %s"
        % r.get("_ship_settle"))
    oe._finalize_rows([r])
    # 배송비 정산 실값(2,910)이 **한 번만**. 고객배송비(3,000)가 아니다.
    assert r["정산예정금(배송비포함)"] == 58430 + 2910, (
        "N열이 배송비를 두 번 더했거나, 수수료 안 뗀 고객배송비를 더했다: %s (배송비 %s)"
        % (r["정산예정금(배송비포함)"], r["배송비"]))
