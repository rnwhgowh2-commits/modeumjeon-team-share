...
3) 같은 탭에 나란히 표시되는가? — **아니다.**
   · 요약 탭 미니카드(정상/고마진/의심손실/계산불가)는 **오직 JS `MR.summarize`**
     (margin_rules.js)가 matched 행에서 직접 집계해 만든다. 서버 summary 에는 이
     분류 카운트가 없다.  (구 재구현본 margin_render.js 는 C3 에서 폐기됨 — 원본
     풀페이지 임베드로 대체. margin_rules.js 는 원본·백엔드 공용으로 존속.)
   · 서버 summary 의 정상매출/정상순마진/정상마진율/이상가건수/정상매입 은 JS 표시
     버킷과 함께 렌더되지 않는다. 표시되지 않으므로 JS 카운트와 모순을 일으킬 표면이
     존재하지 않는다.
