# -*- coding: utf-8 -*-
r"""_isExchangeRow 회귀 —
  2026-09-05 사장님 확정 — "교환"은 간단메모 또는 마켓상태(판매처_주문상태·
...
def test_is_exchange_row_requires_explicit_exchange_evidence():
    ...
    // 1) 더망고 복합 라벨("반품/교환/취소완료")만 있고 판매처_주문상태는 "배송완료" —
    ...
    const withdrawnCancelIsolatedCall = {
      ...
      '판매처_주문상태': '취소철회(구매확정)', '판매처_판매처주문상태': '', '마켓주문상태 (오픈 마켓 연동)': '',
    };
    console.log(JSON.stringify({
      ...
      explicitExchangeMarket: _isExchangeRow(explicitExchangeMarket, 'completed_memo_no'),
    ...
    assert out["explicitExchangeMarket"] is True       # 판매처_주문상태 "교환접수" → 교환
