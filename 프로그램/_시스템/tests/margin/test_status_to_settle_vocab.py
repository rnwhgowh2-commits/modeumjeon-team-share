# -*- coding: utf-8 -*-
"""API 주문상태 → 정산 어휘 번역 + config 버킷 귀속 검증.
...
#   (이전의 config 근사 프록시를 폐기.) 정산 어휘 문자열 하나를 매칭된 행으로 감싸
#   실 정산 판정 → 취소/반품/O 로 환원. classifier 와 정확히 같은 축으로 검증된다.
_SETTLE_MAP = {"O": "O", "X_취소": "취소", "X_반품": "반품", "X_미매칭": "미매칭"}


def _bucket(settle_status: str) -> str:
    """실 classifier 로 정산 어휘 문자열이 어느 정산 버킷인지 판정."""
    row = {"판매처_매칭": True, "판매처_정상건존재": False, "판매처_주문상태": settle_status}
...
def test_lotteon_withdraw_is_cancel_not_settled():
    ...
    assert _bucket("철회") == "O"
    out = S.status_to_settle_vocab("롯데온", "철회")
...
def test_lotteon_collect_confirmed_is_return_not_settled():
    ...
    out = S.status_to_settle_vocab("롯데온", "회수확정")
    assert _bucket(out) == "반품", f"회수확정 → {out}"  # remap 후 반품으로 교정


def test_pinned_values_land_in_settled():
    assert _bucket(S.status_to_settle_vocab("롯데온", "발송완료")) == "O"
    assert _bucket(S.status_to_settle_vocab("옥션", "구매결정")) == "O"
    assert _bucket(S.status_to_settle_vocab("쿠팡", "업체직접배송")) == "O"


def test_identity_values_keep_correct_bucket():
    assert _bucket(S.status_to_settle_vocab("스마트스토어", "배송완료")) == "O"
    assert _bucket(S.status_to_settle_vocab("쿠팡", "결제완료")) == "O"
    assert _bucket(S.status_to_settle_vocab("11번가", "취소완료")) == "취소"
    assert _bucket(S.status_to_settle_vocab("스마트스토어", "반품완료")) == "반품"
    assert _bucket(S.status_to_settle_vocab("롯데온", "회수지시")) == "반품"


def test_exchange_stays_settled_like_original():
    assert _bucket(S.status_to_settle_vocab("스마트스토어", "교환완료")) == "O"
    assert _bucket(S.status_to_settle_vocab("11번가", "교환요청")) == "O"


def test_unknown_passes_through():
    assert S.status_to_settle_vocab("스마트스토어", "듣도보도못한상태") == "듣도보도못한상태"
    assert S.status_to_settle_vocab("스마트스토어", "") == ""
    assert S.status_to_settle_vocab("", "배송완료") == "배송완료"
...
def test_all_known_values_return_string():
    ...
    for mk in ("스마트스토어","쿠팡","롯데온","11번가","옥션","G마켓"):
        for v in vocab:
            out = S.status_to_settle_vocab(mk, v)
