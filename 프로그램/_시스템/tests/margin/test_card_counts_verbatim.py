...
def test_주문취소_교환완료_교환수거완료는_기타가_아니라_완료로_간다():
    """🔴 2026-09-06 라이브 기타 카드 119건 실사 — 11번가 '주문취소'·롯데ON '교환완료'·
    G마켓 '교환수거완료'가 DONE_RTN_PATTERNS 에 없어 종결 상태인데도 기타로 샜다.
    실데이터 없이도 재현되는 최소 단위시험(golden 과 별개, card_kw 없어도 성립)."""
    rows = [
        {'판매처_주문상태': '주문취소', '더망고주문상태 (사용자 연동)': '', '간단메모': ''},
        {'판매처_주문상태': '교환완료', '더망고주문상태 (사용자 연동)': '', '간단메모': ''},
        {'판매처_주문상태': '교환수거완료', '더망고주문상태 (사용자 연동)': '', '간단메모': ''},
    ]
    out = compute_card_counts(rows, source='matched', card_kw={})
    assert out['card_etc'] == 0, out
    assert out['card_completed'] == 3, out


def test_정산여부가_있으면_클레임_텍스트만으로_진행중_카드에_넣지_않는다():
    """2026-09-06 사장님 확정 — settle_status 가 이력(status_at)으로 이미 재판정해
    둔 '정산여부'가 '진행중'이 아니면, 판매처_주문상태 텍스트가 여전히 '반품요청'
    이어도 진행중 카드로 세지 않는다(클레임이 실제로는 받아들여지지 않고 그대로
    배송·구매확정까지 이행된 경우 — 실사례: 취소요청 '26-09-04 → 배송완료 '26-09-06).
    '정산여부' 필드 자체가 없는(계산 안 된) 행은 기존대로 텍스트 판정을 그대로 쓴다."""
    rows = [
        {'판매처_주문상태': '반품요청', '더망고주문상태 (사용자 연동)': '', '간단메모': '',
         '정산여부': '확인불가'},
        {'판매처_주문상태': '반품요청', '더망고주문상태 (사용자 연동)': '', '간단메모': ''},
    ]
    out = compute_card_counts(rows, source='matched', card_kw={})
    assert out['card_inprogress'] == 1, out   # 정산여부 없는 두 번째 행만 진행중


def test_취소요청도_진행중_카드로_잡는다_JS와_동일():
    """🔴 2026-09-06 발견 — '취소요청'이 이 서버 함수의 PROGRESS_PATTERNS에는
    없었다(화면 JS margin_embed.html 은 이미 갖고 있었음 — '취소요청 추가' 커밋이
    서버쪽 갱신을 빠뜨림). 화면에 뜨는 진행중 카드 수와 export/API 값이 갈렸다."""
    rows = [{'판매처_주문상태': '취소요청', '더망고주문상태 (사용자 연동)': '', '간단메모': ''}]
    out = compute_card_counts(rows, source='matched', card_kw={})
    assert out['card_inprogress'] == 1, out
...
#   ★사장님 확정(2026-07-23 2차): "기타뿐 아니라 현지배송완료는 **모두** 이 카드로".
#     그래서 판정을 분류 맨 앞(까대기와 같은 급)에 둔다. 그 대가로 다른 카드에 있던
