...
function _isExchangeRow(r, cardType) {
  ...
  var clean = marketState
    .replace(/반품\/교환\/취소/g, '').replace(/취소\/반품\/교환/g, '')
    .replace(/반품·교환·취소/g, '').replace(/취소·반품·교환/g, '');
  return _matchesAny(clean, subEx);
}
...
function _getRowsByCardFilter_internal(data, type) {
  ...
  var DONE_RTN_PATTERNS = ['반품완료','취소완료','환불승인','회수완료','취소된거래','취소거부','주문취소','교환완료','교환수거완료'];  /* [모음전 2026-09-06] 기타 카드로 새던 11번가·롯데ON·G마켓 3건 — card_counts.py 와 동일 추가. tools/build_margin_embed.py 에도 같은 seam 등록(재생성 시 유지됨) */
