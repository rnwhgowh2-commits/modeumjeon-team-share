...
이 파일이 지키는 것 (2026-08-14 사장님 확정 3·4)
  ...
  ② 🔴 [2026-08-19 지시로 뒤집힘] **「미구성 SKU」 딱지를 이름 칸에 그린다.**
     한 번은 「위상 칸에 이미 사유(축 없음)가 있어 중복」이라 뺐었으나(2026-08-14
     사장님 확정 4), 그건 **위상 칸** 얘기였고 이번 딱지는 **이름 칸**이라 같은
     사실을 두 번 말하는 게 아니다(목록을 훑을 때 위상 칸까지 안 봐도 한눈에 알아
     보게 하는 것이 목적). 판정은 여전히 `unbuilt.py` 하나뿐 — 값은 화면 표시
     여부만 정하고 저장되지 않는다.
...
def test_미구성_딱지를_화면이_그린다(client, 다섯상자):
    """🔴 [2026-08-19 지시로 뒤집힘] 이름 칸에 「미구성 SKU」 딱지가 떠야 한다.

    위상 칸의 「축 없음」 사유와는 다른 자리(이름 칸)에 다른 목적(한눈에 훑기)으로
    붙이는 것이라 중복이 아니다 — 파일 머리말 ② 참조.

    🔴 **왜 렌더된 글자가 아니라 화면 코드를 보나.** 렌더 결과의 「미구성」 글자는
       다른 표본이 남긴 **매트릭스 이름**(`목록_미구성` 등)에도 걸릴 수 있어,
       딱지가 실제로 그려지는지는 템플릿이 그 조건(`{% if b.unbuilt %}`)을
       가진 채 살아 있는지로 본다.
    """
    # 헛돔 방지 — 미구성 표본이 실제로 목록에 올라와 있는지 먼저 본다.
    글 = _글자만(client.get('/optgen/?tab=direct').get_data(as_text=True))
    assert 다섯상자['이름']['미구성'] in 글, '미구성 표본이 화면에 없다 — 시험이 헛돈다'
    assert '미구성 SKU' in 글, '미구성 표본 줄에 「미구성 SKU」 딱지 글자가 안 보인다'
    코드 = _화면코드('webapp/templates/optgen/index.html')
    assert 'b.unbuilt' in 코드, '이름 칸 딱지를 그리는 조건이 템플릿에서 사라졌다'


def test_미구성은_축_없음으로_미완료가_되고_딱지도_같이_뜬다(다섯상자):
    """위상 칸의 사유(축 없음)와 이름 칸의 딱지(unbuilt) 가 **둘 다** 실려야 한다."""
    ...
    # 🔴 [2026-08-19] 이제는 담아야 한다 — 이름 칸 딱지가 이 값을 그대로 읽는다.
    assert x.get('unbuilt') is True, '미구성 SKU 인데 unbuilt 값이 참이 아니다'
