# -*- coding: utf-8 -*-
"""정답지 J~N열 정합 2차 — 2026-07-23 재추출(파일3) 387건 대조로 확정된 규약.

실측 근거(정답지=정답지, 사장님 확정):
· 쿠팡: M열 = 상품정산만(배송비 정산 불포함).
  🔴 N열은 2026-08-13 에 **근거가 교체**됐다 — 여기 있던 「N = M + 고객배송비 **전액**
    (정답지 45건 전수)」는 **정답지의 계산 정의**였고, 마켓이 실제로 주는 돈이 아니었다.
    쿠팡 자기 정산 엑셀(449행·153주문) 전수: 배송료는 독립 정산 행이고 실효 3.3% 수수료가
    붙는다(4,000→3,868 등 124행 예외 0건). 사장님 확정: "실마켓은 100% 정답이야."
    → N = M + 배송비 정산 실값. 자세한 판정은 `order_export.CP_SHIP_FEE_FACTOR` 주석.
    M열 규약(상품분만)은 **그대로 유효**하다 — 이중 계상 방지선이라 여기서 계속 지킨다.
· G마켓: K열(실결제) = 단가×수량 **원금**(판매자 쿠폰 할인 전) — 13/13 전수.
  할인 있던 12건이 전부 (정답지K=원금, 우리K=할인후) 로 어긋났고 M열은 이미 일치.
· 스스: CANCELED_BY_NOPAYMENT 가 영문 원코드 그대로 노출 + zero_cancel 미적용
  (정답지 '취소완료(미결제)' K=원금 43,000 vs 우리 33,900 실측).
· 쿠팡 반품완료·롯데온 취소요청/철회: 정답지 K열 = 원금(단가×수량) — 정산(M)은 건드리지 않음.
"""
import copy
...
def test_쿠팡_추정_정산은_상품분만_배송비는_N열에서만(monkeypatch):
    """M열은 상품분만(불변). N열은 배송비도 수수료를 뗀 값(2026-08-13 근거 교체).

    기존엔 M에 배송비×0.97 이 더해져 +4,014 씩 어긋났다(3건) — 그 규약은 그대로 지킨다.
    """
    ...
    # 미정산이라 배송비도 추정 — 4,000 전액이 아니라 3,868(실효 3.3% 차감)
    assert fin["정산예정금(배송비포함)"] == (round(25100 * 0.8845)
                                             + round(4000 * oe.CP_SHIP_FEE_FACTOR))
    assert fin["배송비"] == 4000        # L열(고객이 낸 배송비)은 안 건드린다


def test_쿠팡_실정산도_상품분만_배송비정산은_더하지_않는다(monkeypatch):
    """M에 배송비정산이 섞이면 N이 이중 가산된다 — M은 끝까지 상품분만.

    배송비 정산 실값(3,880)은 M 이 아니라 **N에서만** 쓰인다.
    """
    rows = _cp_rows(monkeypatch, copy.deepcopy(_BOX),
                    settle={("OID-1", "VI-1"): 22000}, deliv={"OID-1": 3880})
    assert rows[0]["정산예정금액"] == 22000
    assert rows[0]["_settle_source"] == "real"
    fin = oe._finalize_rows([dict(rows[0])])[0]
    assert fin["정산예정금액"] == 22000                    # M 불변 — 이중 계상 방지선
    assert fin["정산예정금(배송비포함)"] == 22000 + 3880   # 실값을 쓴다(4,000 아님)


# ── ESM(옥션·G마켓): 실결제 = 원금(단가×수량+옵션추가금) ────────────────────────

def test_G마켓_실결제는_원금으로_통일():
    """정답지 13/13 전수: K열=단가×수량(쿠폰 할인 전). 할인 반영값(BuyerPayAmt)을 덮는다."""
    r = oe._finalize_rows([{
...
def test_스스_미결제취소는_정산0_실결제_원금():
    """실측 913547351: 정답지 K=43,000(원금)·정산 없음 vs 우리 K=33,900·M=31,866(추정 날조)."""
    r = oe._finalize_rows([{
...
def test_취소요청_실결제는_원금_정산은_불변():
    """실측 616897117: 정답지 K=138,100(원금) — 취소요청(철회 포함)도 원금 표기."""
    r = oe._finalize_rows([{
...
def test_쿠팡_반품완료_실결제는_원금():
    """실측 749312893: 정답지 K=52,200(단가) vs 우리 29,200(할인후). 정산(공란)은 그대로."""
    r = oe._finalize_rows([{
...
def test_11번가_미정산_M은_stlPlnAmt에서_배송비를_뺀다(monkeypatch):
    ...
    — 정답지 M열 29,913 = stlPlnAmt − dlvCst. N열은 _finalize 가 +배송비로 복원."""
...
def test_11번가_K는_적용할인_기준(monkeypatch):
    ...
    정답지 K=28,400 은 '적용' 할인(tmallApplyDscAmt 4,400) 기준 = ordAmt−적용할인.
    ...
    assert r["실결제금액"] == 28100 + (4700 - 4400)   # 28400 = 정답지 K
...
def test_쿠팡_배송비는_도서산간_추가비를_포함한다(monkeypatch):
    ...
    (remoteArea=True) — 정답지 배송비 5,000. remotePrice 를 안 더하면 L·N열 누락."""
...
def test_ESM_추정은_원금기준_비율(monkeypatch):
    """실측(2026-07-23 G마켓 4471677631): 정답지 M=원금×0.87 인데 우리 추정이 +3,041 —
    ...
    assert row["정산예정금액"] == round(40200 * (72645 / 83500))   # 34974 = 정답지 실측
    s.close()
...
def test_ESM_추정_시장비율은_최빈값(monkeypatch):
    ...
    이력의 중앙값(라이브 실측 0.85 오염)이 아니라 최빈 구간을 쓴다(정답지=0.87 일치)."""
    from lemouton.markets import line_uid as L
    from lemouton.markets import order_store as OS
    s = _sess()
    hist = []
...
def test_낡은_11번가_배송완료_스냅샷을_byno로_갱신(monkeypatch):
    """11번가는 배송 후에도 정산예정금(stlPlnAmt)을 갱신한다(T-쿠폰 등 — 정답지 대조
...
def test_옵션이_다른_저장분_라인의_금액은_채우지_않는다():
    ...
    다른데 주문번호 채움이 그 라인의 단가 39,000을 붙임(실제 39,900 — 정답지 대조 발견).
...
def test_11번가_적용할인이_표기보다_크면_그만큼_뺀다(monkeypatch):
    ...
    086115548 검산: ordAmt 23,300 − 적용 2,669 = 20,631(정답지) = ordPayAmt 23,790 − 배송비
    ...
    r = _e11_rows(monkeypatch, od)[0]
    assert r["실결제금액"] == 23790 - 3000 + (2510 - 2669)   # 20631 = 정답지
