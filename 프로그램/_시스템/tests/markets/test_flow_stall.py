...
def test_반품_교환_취소는_감시_대상이_아니다():
    """되돌아오는 중이거나 끝난 주문은 「배송이 안 움직인다」가 아니다.

    2026-07-24 라이브 실측 — 이걸 안 빼면 15건 전부 거짓 경보였다
    (반품완료 12 · 회수지시 3). 진짜 멈춘 주문은 0건이었다.
    """
    for st in ("반품완료", "반품요청", "교환요청", "교환완료", "취소완료",
               "취소요청", "회수지시", "회수진행", "회수완료", "철회"):
        v, _ = fs.judge(_row(주문상태=st), NOW)
        assert v == "moving", st
...
def test_송장칸에_번호가_아닌_문구가_들어와도_속지_않는다():
    """정답지는 송장열에 '송장입력됨' 같은 상태 문구를 넣는다 — 번호가 아니다."""
