...
def test_기존_초안이_수집_금지어에_걸리면_크롤_갱신도_저장하지_않는다(client, url_bag):
    ...
    # 2) 소싱처가 재크롤됐다 — 상품명이 금지어를 달고, **크롤 소유 칸**(재고·분류)도 바뀌었다.
    #    ★ name 은 갱신 시 절대 안 덮는 칸이라(draft_from_crawl.py:432 — 비었을 때만) name
    #      만 단언하면 expire 를 없애도 통과한다(가짜 방어). 그래서 crawl-owned 칸을 본다.
    s = SessionLocal()
    try:
        from lemouton.sources.models import SourceProduct
        sp = s.query(SourceProduct).filter_by(url=bad_url).first()
        sp.product_name = '이월상품 스니커즈'
        sp.last_stock = 999                    # 원래 5 (crawl-owned: draft.stock_quantity)
        sp.category_path = '가방>백팩'          # 원래 신발>스니커즈 (draft.source_category_path)
        s.commit()
    finally:
        s.close()
    ...
    try:
        ...
        # ★ crawl-owned 칸이 새지 않았는지 — expire 가 이 갱신을 되돌려야 한다.
        assert d.stock_quantity == 5, f'재고 999 가 샜습니다: {d.stock_quantity}'
        assert d.source_category_path == '신발>스니커즈', \
            f'분류 「가방>백팩」이 샜습니다: {d.source_category_path}'
    finally:
        s.close()
