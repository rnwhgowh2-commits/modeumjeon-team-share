...
EXPECTED = {
    ...
    # [2026-07-19 기준선 갱신 — 의도한 동작 변경] 선반영(preapplied) 건너뛰기 폐지.
    #   가이드의 '선반영' 표기와 실제가 어긋나 있었다(무신사 표면가 salePrice 는 등급할인
    #   **적용 전** 값 → 빼는 게 맞다). 선반영/후반영 구분을 없애고 전부 차감한다.
    #   순서는 정액(원) → 정률(%) 만. 사용자 확정 2026-07-19.
    #   변화: 선반영쿠폰 5,000 이 이제 차감됨 → 9,000 → 4,500.
    ...
    # [2026-07-19 기준선 갱신] tagged 경로도 동일 — 선반영 건너뛰기 폐지 → 전부 차감.
