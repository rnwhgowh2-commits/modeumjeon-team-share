...
# 실크롤 결합행 회귀: '기본 적립 등급적립 3,420원 후기적립 2,500원' 한 줄에서 후기적립=2,500
#   (트리거 '뒤' 금액). 라인-전체 최대면 3,420(등급적립) 오추출 → 매입가 틀림. (dev E2E서 발견)
...
def test_combined_row_extracts_amount_after_trigger_not_line_max():
    on = _enabled(_musinsa_effective_from_crawl(GUIDE, EXCL, {"lines": ACC2_COMBINED}))
    # 후기적립=2,500 (결합행의 3,420 아님), 결제적립=11,030(택1 최대), 구매적립=3,420
...
def test_combined_row_final_price():
    ...
    assert res["final_price"] == 105950  # 122900 - 3420 - 2500 - 11030
