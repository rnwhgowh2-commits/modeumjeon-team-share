...
def test_lotteimall_point_rewards(sess):
    ...
    # [2026-07-23 L.POINT 기준 정정] 88,800 → **88,900** (의도된 변경).
    #   적립 기준을 주문서 실측(쿠폰 차감 후 금액)에 맞추려 정액→정률로 환산하면서
    #   적립이 **정액 뒤**로 밀렸다: 100,000 −7,000(카드) −100(리뷰) = 92,900
    #   → ×2%(=2,000/100,000) = 1,858 → 91,042 → 캐시백 2,048 → 88,994 → 88,900.
    #   카드 청구할인이 있는 상품은 적립 기준이 실제(카드 전)보다 낮아져 **덜 깎인다**
    #   = 매입가 과대 = 안전 방향. 실측 케이스(카드 없음)에서는 정확하다.
    assert got['final'] == 88900
