...
[중요] **훑는 일은 서버가 하지 않는다.** 페이지를 여는 건 로컬 PC 의 크롬 확장이다
...
def expected_ext_version():
    ...
    [중요][중요] [2026-08-13 라이브 실측] 캐시에 **만료가 없어서** 배포한 뒤에도 옛 값을
...
def _ver_tuple(v):
    ...
    [중요] **글자로 견주면 안 된다** — `"0.8.9" > "0.8.10"` 이 참이 된다(9 > 1).
    """
...
def create_search_filter():
    ...
    [중요] **규칙을 모르는 소싱처는 여기서 막는다.** 만들게 두면 「지금 수집」이 영원히
...
def compute_price_for(session, source_product, policy_id):
    ...
    ━━ 산식은 한 줄도 여기 쓰지 않는다 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      [중요] 같은 숫자를 두 곳에서 만들면 반드시 갈린다.

    [중요] **못 정하면 0 이 아니라 None** 을 돌려준다. 호출부가 판매가를 안 넣고,
      사전 점검이 「판매가가 0 이하입니다」로 막는다. 아무 값이나 채우면 그 가격이
      그대로 마켓에 나간다(돈이 걸린 자리라 폴백 금지).

    [중요] **못 정한 이유를 반드시 함께 돌려준다.** 예전엔 `except` 가 예외를 삼켜
...
def filter_scorecard(filter_id: int):
    ...
    ━━ 지키는 것 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    [중요] 매출 기준은 `정산예정금(배송비포함)` — `orders/fulfillment.SETTLE_FIELD` 를
      **그대로** 쓴다. 수수료율로 되계산하면 「에러 없이 틀린 숫자」가 된다.
    [중요] **금액을 못 읽은 주문을 0 원으로 세지 않는다.** 0 으로 뭉개면 「안 팔렸다」와
      「팔렸는데 금액을 모른다」가 같아져, 잘 되는 필터를 꺼 버릴 수 있다.
      → `sales_unknown_lines` 로 따로 센다.
    [중요] 마켓 상품번호를 뽑는 규칙은 `orders.price_diff` 가 이미 안다 — 여기서 다시
...
def build_from_filter(filter_id: int):
    ...
    [중요] **자동으로 하지 않는다.** 검색 한 번에 수백~수천 주소가 들어오는데 찾자마자
...
def edit_search_filter(filter_id: int):
    ...
    **찾아 둔 주소를 다시 훑어야 한다**(라이브 필터는 이미 30개를 찾아 뒀다).
    ...
    [중요] **안 보낸 칸은 안 건드린다.** 일부만 고치려다 나머지가 비워지면
      조용한 데이터 손실이다. `payload 에 그 키가 있을 때만` 바꾼다.

    [중요] **소싱처와 검색 주소는 못 고친다.** 그걸 바꾸면 이미 찾아 둔 주소가
...
def delete_search_filter(filter_id: int):
    """[중요] **소프트 삭제만.** 이 필터로 찾은 상품은 절대 건드리지 않는다.
