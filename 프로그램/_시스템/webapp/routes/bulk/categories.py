...
자동으로 이어받기가 된다(새 버튼 없음). 자식이 DB 에 '일부만' 저장된 채 죽은 비-리프
노드는 안전하게 추측하지 않고 다시 fetch 한다(`category_harvest.py` 의 `harvest_coupang`
docstring 우려사항 참조 — 완전 무손실 보장은 아니다).
"""
...
def _build_coupang_known(session):
    ...
    걷느라 진도가 안 나간다. parent_code 관계로 "이 노드의 자식이 DB 에 있는가"를 판정해
    known[code]['children'] 에 채운다 — `harvest_coupang` 은 is_leaf=True 이거나 children 이
    채워진 노드만 fetch 를 건너뛴다(자세한 안전 조건은 `category_harvest.harvest_coupang`
    docstring 참조). 빈 테이블(첫 수집)이면 빈 dict 를 돌려줘 기존 전체 탐색과 동일하다.
    """
