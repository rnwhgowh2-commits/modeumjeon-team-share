...
[중요] 값을 지어내지 않는다 — 모르면 None(화면 「확인 불가」/「—」).
[중요] 같은 값을 다시 계산하지 않는다 — 전부 기존 단일 진실 원천을 **호출만** 한다:
...
def stages_for(session, codes: list[str]) -> dict:
    ...
    [중요] 옵션관리(/matrix)·옵션 목록(/optgen)·상품관리가 같은 함수를 쓴다 —
...
def policy_models(s, codes: list[str]) -> set:
    ...
    [중요] 상품 단위(BundlePolicyLink)만 보면 「구성(벌)마다 다른 정책」을 붙인 상품이
...
def _build_sales_index(s, days: int) -> dict:
    ...
    · 매입가 원천은 `order_line_purchases` 하나(`markets/purchase_price.get_many`).
      [중요] `resolve_purchase_price` 를 쓰지 않는다 — 그건 없을 때 사입가·소싱 예상가로
    ...
    · [중요] **0 으로 채우지 않는다.** 실매입가 없는 줄은 `pp_missing` 으로 세고,
...
def sales_index(days: int = 30, *, fresh: bool = False) -> dict:
    ...
    [중요] 단 **실매입가가 바뀐 것은 예외** — 그건 「낡음」이 아니라 「틀림」이다.
...
def bundle_list():
    ...
    [중요] 예전에는 `판매 중 = display_no 있는 것`이었다. 상품번호는 만들 때 무조건 붙어서
...
def tower_sales(code: str):
    ...
    · 실현 마진 = 정산 예정 − **실매입가**(설계서 §6.2·3단계). 산식·거르개는
      `_build_sales_index` 주석 참고. [중요] 실매입가가 없는 줄은 0 으로 채우지 않고
      `pp_missing` 으로 세어 화면이 「매입가 미입력 N건」이라 말하게 한다.
      쓴 줄 수(`realized_basis`)도 같이 내보내 「N건 중 B건 기준」으로 밝힌다.
    · [중요] 예상가·사입가로는 실현 마진을 만들지 않는다 — 그건 「예상」이지 실적이 아니다.
...
def _coupon_status(s, code: str) -> dict:
    ...
    [중요] `have` 와 `want` 를 **따로** 준다. 둘이 다를 수 있고(예: 아직 안 걸림),
