...
def _parse_markets(args, dropped=None):
    ...
    [중요] 걸러진 마켓 이름을 `dropped` 리스트에 담아 준다(2026-08-12). 옛 판은 조용히
...
def orders_diag_esm_timing():
    ...
    [중요] 왜 있나 — 2026-08-13 클레임 49회를 동시에 보내도록 고쳤는데 오히려 느려 보였다.
...
def orders_fulfillment_recheck():
    ...
    [중요] 여기서 긁지 않는다 — 크롤은 사장님 PC 확장 몫이다(서버는 IP 가 다르다).
...
def _invalidate_tower_sales(reason: str) -> None:
    ...
    [중요] 왜 필요한가(라이브 실측 2026-08-06) — 판매 이력은 300초 서버 캐시라, 여기서
...
def purchase_price_bulk():
    ...
    · 상태 열 일괄 지정(`/api/line-status/bulk`)과 **같은 규약**이다 — 열쇠는 반드시
      `line_uid`. [중요] 주문번호로 묶으면 다품목 주문의 형제 줄까지 같은 값이 박힌다.
    · `price` 가 비었거나 0 이면 **고른 줄의 실매입가를 지운다**(= 「입력 안 함」).
      한 줄 저장과 같은 규칙이라 「빈칸으로 저장 = 지움」이 화면마다 갈리지 않는다.
    · [중요] 한 줄이 실패해도 나머지를 되돌리지 않는다 — 대신 실패한 줄을 **그대로 돌려준다**
...
def purchase_price_resolve():
    ...
    [중요] 판정(`flags`)을 **여기서 같이** 내는 이유 — 판정은 「주문 행 + 그 매입가」의 순수
...
def supply_mode_bulk():
    ...
    [중요] 열쇠는 반드시 line_uid — 주문번호로 묶으면 다품목 주문의 형제 줄까지 같이 바뀐다.
    """
    from lemouton.markets import supply_mode as _sm
...
def status_options_list():
    ...
    [중요] 비어 있는 게 정상이다 — 화면은 그때 「+ 첫 항목 만들기」를 안내한다.
    """
...
def status_options_delete(option_id):
    ...
    [중요] 쓰는 중이면 **409 + 몇 건이 쓰는지**로 거절한다 — 화면이 「3건이 쓰는 중」
...
def line_status_resolve():
    ...
    [중요] 저장 안 된 줄에는 **기본 항목을 얹어 보내되 `is_fallback: true`** 다 —
...
def purchase_price_upload_mango():
    ...
    · [중요] 못 붙은 행·후보가 여럿인 행은 버리지 않고 응답에 담는다(화면이 목록으로 보여 준다).
...
def settle_plan_detail():
    ...
    [중요] 집계와 **같은 판정(SP.resolve)** 을 쓴다 — 예전엔 여기만 classify 로 걸러
...
def _settle_plan_detail_order(SP, market, account, bucket, unit):
    ...
    [중요] 「매출액을 실결제금액 대신 상품금액+배송비로 **대체**」한 줄은 목록에도 그 사실을
...
def settle_plan_export():
    ...
    [중요] 왜 필요한가(2026-08-06 라이브) — 「이미 받은 것 3.52억」·「받았을 것 1.52억」이
...
def orders_invoice_sweep_run():
    ...
    [중요] 왜 필요한가(2026-07-30 실측) — 저장분 송장 보유율이 G마켓 34/190 · 옥션 25/47 ·
    ...
    [주의] ESM 은 5초/1콜이라 기간이 넓으면 오래 걸린다(클플 100초 상한 주의 — 나눠 돌릴 것).
    """
    from flask import jsonify
    market = (request.args.get('market') or '').strip()
...
def orders_diag_esm_delivery_settle():
    ...
    `?market=gmarket&from=YYYY-MM-DD&to=YYYY-MM-DD&srch=D1,D3,D6&alias=`
      ...
      · [중요] 당일·미래 날짜로 조회하면 Error 414 — 어제까지로 물어야 한다(지도 코드표).
    ...
    **마켓이 실제로 준 값**을 봐야 한다(원문 안 보고 「필드가 없다」고 단정 금지).
    ...
    지도(esm 주문조회) 확정 필드:
      ...
      · `AcntMoney`   (판매단가×수량)+옵션가 − **판매자 할인금액 총액** − 판매자 지급
                      스마일캐시 − 사이트 할인금액 + 배송비
        [주의] 배송비는 **장바구니 합계**로 내려온다(G마켓은 1개 주문번호에만, 옥션은 모든
    ...
    `?market=auction&days=14&limit=40&alias=` — 기간 훑기(느림).
      [중요] ESM 주문조회는 **5초당 1회** 제한이라(ResultCode=3000) 기간이 넓으면
...
def orders_diag_coupang_settle_parity():
    ...
    [중요] 왜(2026-08-06 사장님: "이걸 놓치면 엄청난 정산 금액 차이") — 우리 화면 금액은
      주문별 정산액이다. 마켓이 인정한 **정산대상액**과 같은지 스스로 검산할 창구가 없었다.

    [중요] 대조 상대를 한 번 갈아탔다 — 처음엔 회차 finalAmount(통장 입금액)와 맞댔더니
...
def orders_diag_rg_rounds():
    ...
    [중요] 왜(2026-08-13 사장님 지적) — 화면 「지급 예상금액」이 우리 숫자와 다르다.
    ...
    [중요] 합계만 비교하면 「우연히 비슷」과 「정말 같음」을 못 가른다. 회차(정산일·비율)마다
...
def orders_diag_stale_delivered():
    ...
    [중요] 왜 필요한가(2026-08-12) — 자동 틱을 얹은 지 4일인데 롯데온 미확정이 622→579건,
...
def orders_diag_eleven11_settle():
    ...
    [중요] 왜 필요한가(2026-08-07) — 문서·지도에는 `stlDy`(정산일, [필수])가 있어 그걸
...
def orders_diag_e11_seller_dc():
    ...
    [중요] 왜 필요한가(2026-08-13) — 배송중·배송완료·구매확정 주문은 **목록조회가
...
def orders_diag_e11_order_raw():
    ...
    [중요] 왜 필요한가(2026-08-13) — 지도에 `sellerDscPrc` 가 있다는 것은 **「칸이 있다」**이지
...
def order_view_prefs():
    ...
    [중요] 실패를 조용히 삼키지 않는다 — 저장이 안 됐는데 성공한 척하면 사장님이
...
def settle_recon_run():
    ...
    [중요] 열 이름을 추측하지 않는다 — 금액 열을 못 찾으면 파일에서 본 열 이름을 그대로
...
def settle_recon_run_live():
    ...
    [중요] 왜(2026-08-13) — 스마트스토어 대조는 「기준일 규칙만 코드에 넣고 실행은 한 번도
...
def settle_recon_run_manual():
    ...
    [중요] 왜 이 칸이 필요한가(2026-08-13 실측) — 쿠팡 「미구매확정」 상세 엑셀
    ...
    [중요] 이 숫자는 **대조 상대**일 뿐, 우리 정산액이 되지 않는다. 돈 값의 원천은 끝까지
...
def orders_lotteon_paid_context():
    ...
    [중요] 왜 `trNo` 를 서버가 주나(2026-08-13 라이브 실패) — 확장이 셀러오피스 **화면에서**
    ...
    [중요] 왜 「최근 가져온 내역」인가(사장님 지적) — 단추를 눌러도 **언제 · 얼마나 들어왔는지**
