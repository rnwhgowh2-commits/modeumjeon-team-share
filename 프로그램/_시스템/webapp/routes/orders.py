...
def orders_settlement_sweep_run():
    """옥션·G마켓·쿠팡·롯데온 저장분의 정산액을 마켓 실값으로 갱신(주문 조회 없음).

    스케줄러가 최근 60~75일을 자동으로 훑지만, 그 전에 이미 고착된 과거분은 한 번
    ...
    쿠팡·롯데온도 지원한다(`?market=coupang` / `?market=lotteon`). 단 이 둘은 from/to 가
    **인식일(구매확정일)** 창이다(주문일이 아니다) — 정산이 구매확정 뒤 인식되므로
    옛 주문도 최근 인식창이 덮는다.
    """
    from flask import jsonify
