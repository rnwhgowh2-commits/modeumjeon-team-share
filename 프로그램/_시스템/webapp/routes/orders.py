...
@bp.route('/_probe/eleven11')
def _probe_eleven11():
    """[임시·진단] 11번가 주문/정산 API를 서버에서 실호출 → 응답 '필드 이름만' 반환.

    개인정보 값은 절대 반환하지 않음(태그명·빈도만). 저장된 키로 실제 스키마 확인용.
    스펙 확정 후 삭제 예정.
    """
    from flask import jsonify
    from shared.platforms.eleven11.orders import _parse, _localname
    ...
    def _fields(xml_text):
        try:
            ...
            return {"parse_error": f"{type(e).__name__}: {str(e)[:200]}",
                    "head": (xml_text or "")[:200]}
        if root is None:
            return {"empty": True, "head": (xml_text or "")[:200]}
        counts = {}
        for el in root.iter():
            ln = _localname(el.tag)
            counts[ln] = counts.get(ln, 0) + 1
        return {"root": _localname(root.tag), "field_counts": counts}

    client = _oe._account_client("eleven11")
    if client is None:
        return jsonify(ok=False, error="11번가 계정 클라이언트 생성 실패(키 미설정?)"), 400
    until = _dt.datetime.now(_oe.KST)
    since = until - _dt.timedelta(days=7)
    out = {}
    # 1) 주문(발주확인 기간별 결제완료)
    try:
        opath = "/rest/ordservices/complete/{}/{}".format(
            since.strftime("%Y%m%d%H%M"), until.strftime("%Y%m%d%H%M"))
        out["order"] = {"path": opath, **_fields(client.request("GET", opath))}
    except Exception as e:  # noqa: BLE001
        out["order"] = {"error": f"{type(e).__name__}: {str(e)[:300]}"}
    # 2) 정산(정산내역조회)
    try:
        spath = "/rest/settlement/settlementList/{}/{}".format(
            since.strftime("%Y%m%d"), until.strftime("%Y%m%d"))
        out["settlement"] = {"path": spath, **_fields(client.request("GET", spath))}
    except Exception as e:  # noqa: BLE001
        out["settlement"] = {"error": f"{type(e).__name__}: {str(e)[:300]}"}
