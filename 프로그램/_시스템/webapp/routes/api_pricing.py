...
@bp.route('/_diag/dedup-source-options')
def _diag_dedup_source_options():
    """[임시 진단·수정] SourceOption 포맷-중복 정리(안전 규칙).
    근본원인: 같은 옵션이 크롤 경로마다 다른 색 포맷(사이즈-in-색 / 상품명 / 정상색)으로
      INSERT 돼 (product,size)당 다중 행 → 매트릭스가 null 행 읽고 상품 999 폴백.
    규칙(오삭제 차단): (product, 정규화size) 그룹에서 **모든 행이 색 호환**일 때만
      non-null·freshest 1개 유지, 나머지 soft-delete. 색 호환 = 한쪽이 색없음/사이즈-in-색
      이거나, 둘 다 진짜색이면 한쪽이 다른쪽을 포함(같은색의 다른 표기). 진짜 다른 색이
      섞이면(멀티색 색상모음전) 그룹 통째 skip. apply=1 실행 / 아니면 dry-run. ?site=lotteon 필터."""
    import datetime as _dt
    import re as _re
    apply = request.args.get('apply') == '1'
    site_filter = request.args.get('site') or None

    def _is_size_in_color(x):
        x = (x or '').strip()
        if not x:
            return True
        rest = _re.sub(r'[\d\s]|mm', '', x, flags=_re.I).strip()
        return bool(_stk_digits(x)) and not rest

    def _color_compat(a, b):
        if _is_size_in_color(a) or _is_size_in_color(b):
            return True
        an = (a or '').strip()
        bn = (b or '').strip()
    ...
    try:
        ...
        if site_filter:
            q = q.filter(SourceProduct.site == site_filter)
        sps = q.all()
        now = _dt.datetime.now(_dt.timezone.utc)
        _MIN = _dt.datetime.min.replace(tzinfo=_dt.timezone.utc)
        rep = {'dupe_groups': 0, 'skipped_multicolor': 0, 'soft_deleted': 0,
               'products_touched': 0, 'samples': []}
        for sp in sps:
            rows = (s.query(SourceOption)
                    .filter(SourceOption.source_product_id == sp.id,
                            SourceOption.deleted_at.is_(None)).all())
            groups = defaultdict(list)
            for r in rows:
                nsz = _stk_digits(r.size_text) or _stk_digits(r.color_text)
                if not nsz:
                    continue
                groups[nsz].append(r)
            touched = False
            for nsz, grp in groups.items():
                if len(grp) <= 1:
                    continue
                compat = all(_color_compat(grp[i].color_text, grp[j].color_text)
                             for i in range(len(grp)) for j in range(i + 1, len(grp)))
                if not compat:
                    rep['skipped_multicolor'] += 1
                    continue
                rep['dupe_groups'] += 1

                def _ua(r):
                    return r.updated_at if r.updated_at else _MIN
                grp_sorted = sorted(
                    grp, key=lambda r: (r.current_stock is not None, _ua(r)),
                    reverse=True)
                keeper = grp_sorted[0]
                dels = grp_sorted[1:]
                for r in dels:
                ...
                if len(rep['samples']) < 12:
                    rep['samples'].append({
                        'spid': sp.id, 'size': nsz,
                        'keep': '#%s(stk=%s,col=%r)' % (keeper.id, keeper.current_stock, keeper.color_text),
                        'del': ['#%s(stk=%s,col=%r)' % (r.id, r.current_stock, r.color_text) for r in dels],
                    })
            if touched:
                rep['products_touched'] += 1
        if apply:
            s.commit()
        return jsonify(ok=True, applied=apply, site=site_filter, **rep)
    finally:
        s.close()
