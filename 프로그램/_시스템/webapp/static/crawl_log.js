...
(function () {
  ...
  function renderHeader() {
    ...
    // [2026-07-08] '오늘 바퀴 +1'(pass-done)은 이제 페이지가 보내지 않는다 — 확장(background.js
    //   runQueueBG)이 한 패스 완료당 정확히 1회 보내는 유일한 신고자다. 페이지도 같이 보내면
    //   한 바퀴 완료 순간 두 신고가 ~100ms 안에 도착해 서버가 회차를 2개 박았다(중복쌍 근본원인).
    //   신고자를 확장 하나로 단일화 → 중복 원천 불가. (과거 중복은 서버가 전수 dedupe.)
