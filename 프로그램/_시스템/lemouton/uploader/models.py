...
class PriceSnapshot(Base):
    ...
    #   구성마다 정책이 달라 값이 갈리므로, 다음 사이클의 기준선(「바뀐 것만 보낸다」)도
    #   구성별로 갈라야 한다. 안 가르면 두 벌이 서로를 덮어써 같은 값을 계속 다시 보낸다.
    #   NULL = 구성 축이 생기기 전에 남은 옛 줄. 기준선 조회 때 같이 본다.
    #   FK 를 걸지 않는다 — 구성을 지워도 **왜 그 값으로 올렸나**의 기록은 남아야 한다.
