...
개통(정본 스펙 2026-07-17): update_price_and_stock 이 ①재고조회(prodmarket/stocks)로 대상
재고번호(prdStckNo)의 optWght 확보 ②PUT prodservices/stockqty/{prdStckNo} 로 그 재고번호
1건만 안전 변경(full-replace 아님) ③상품 단위 가격 전송. market_option_id=prdStckNo.
...
class Eleven11Adapter(MarketAdapter):
    ...
    def update_price_and_stock(self, *, canonical_sku, market_product_id,
        ...
        11번가 재고는 **재고번호(prdStckNo) 단위**로 안전하게 바꾼다(full-replace 아님).
        market_option_id = prdStckNo(재고번호). ①현재 재고조회로 그 재고번호의 optWght 를
        확보 → ②PUT stockqty 로 대상 1건만 변경 → ③상품 단위 가격 전송.

        안전장치:
          · 재고조회 실패/대상 재고번호 미발견 → 전송 안 함(실패 반환).
          · 재고 성공 후에만 가격 전송. 둘 중 하나라도 실패면 실패로 표면화(부분전송 표시).
        """
        ...
        target = str(market_option_id)   # = prdStckNo

        # ① 현재 재고조회 → 대상 재고번호의 optWght 확보(변경 시 echo-back)
        try:
        ...
        match = [o for o in current if str(o.get("prd_stck_no")) == target]
        if not match:
            return UploadResult(
                market="eleven11", canonical_sku=canonical_sku, success=False,
                error=f"대상 재고번호(prdStckNo={target}) 미발견 — 전송 중단")
        opt_wght = match[0].get("opt_wght")
        # ② 재고번호 1건 재고수량 변경(안전 — 타 옵션 무관)
        try:
            sr = update_stock_by_stock_no(prd, target, int(new_stock), opt_wght, client=client)
        except Exception as e:  # noqa: BLE001
            return UploadResult(market="eleven11", canonical_sku=canonical_sku,
