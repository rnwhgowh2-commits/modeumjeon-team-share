...
def refresh_settlement_coupang(*, since=None, until=None,
    ...
    # 🔴 [2026-08-13] 배송비 정산 실값 {orderId: 금액} — 여태 `_deliv` 로 받아서 **버렸다**.
    #   스윕은 60~75일 묵은 주문을 추정→실값으로 바꾸는 바로 그 경로다. 여기서 안 실으면
    #   M 만 실값이 되고 N열은 고객배송비 **전액**을 더해, 과거분의 「받을 돈」이
    #   배송비 수수료만큼 계속 과대다(쿠팡 실측 4,000 → 132 → 3,868 · 라이브 2,072건).
    deliv_map: dict = {}
    ...
    for name, cli in _esm_settlement_clients("coupang"):
        ...
        for k, v in (_deliv or {}).items():
            deliv_map.setdefault(str(k), v)       # 배송비 정산 실값 — 첫 계정 우선
