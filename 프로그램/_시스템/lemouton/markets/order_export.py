...
# 🔴 정산예정금액(M열) = **상품분만**. 배송비 정산은 여기 안 들어간다.
#   배송비 정산(쿠팡 deliveryFee.settlementAmount·스스 DELIVERY행)은 `_ship_settle` 로
#   따로 실어 **N열(정산예정금(배송비포함))에서만** 더한다.
...
COLUMN_META = {
    ...
    # 🔴 종전 「정산예정금액 + 고객배송비」 — 이제 마켓이 준 배송비 정산 실값(수수료 뗀 값)을 더한다.
    "정산예정금(배송비포함)": {"kind": "calc", "desc": "정산예정금액 + 배송비 정산(마켓이 준 실값 = 수수료 뗀 값. 없으면 고객배송비 전액 추정)"},
}
