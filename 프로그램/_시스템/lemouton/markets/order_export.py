...
# 🔴 M열(`정산예정금액`) = **상품 정산만**. 배송비 정산은 안 섞는다.
#   배송비 정산은 행의 내부 키 `_ship_settle` 로 실려 N열(`정산예정금(배송비포함)`)에서만
#   쓰인다 — 마켓이 배송비에서도 수수료를 뗀다(쿠팡 실측 3.3%, 2026-08-13 엑셀 전수).
#   실값 출처: 쿠팡 deliveryFee.settlementAmount·스스 DELIVERY행. 없는 마켓은 고객배송비(옛 규약).
#   ★ 여기 「정산예정금액 = 상품정산 + 배송비정산」이라고 적혀 있었다. 그 글을 믿고
#     M 에 배송비를 넣으면 N 이 또 더해 이중 계상된다 — 2026-08-07 에 실제로 났던 사고다
#     (스스 5건 14,550원 과다 · 수수료율 1.32% 오표시). `_finalize_rows` 주석 참고.
...
COLUMN_META = {
    ...
    # 🔴 「상품정산 + 배송비정산」이라고 적혀 있었다 — **금지된 정의**를 화면이 광고했다.
    #   M 에 배송비를 넣으면 N 이 또 더해 이중 계상된다(2026-08-07 사고 · 2,910원 과다 ·
    #   수수료율 1.32% 오표시). 「안 들어간다」를 적어야 다음 사람이 또 안 넣는다.
    "정산예정금액": {"kind": "calc", "desc": "상품분 정산(마켓 수수료 차감). 배송비는 안 들어간다 → 「정산예정금(배송비포함)」 열"},
    ...
    # 🔴 「실결제 − 정산예정금액」 한 갈래만 적혀 있었다 — 코드는 마켓이 준 실값을 먼저 쓴다
    #   (:preset_fee · 롯데온 SettleCommission). 설명이 코드와 딴 말을 하고 있었다.
    "마켓수수료":   {"kind": "calc", "desc": "마켓이 실값을 주면 그 값(롯데온 등), 없으면 실결제 − 정산예정금액"},
    # 🔴 분모는 2026-08-06 에 총주문금액(할인 전) → 실결제(할인 후)로 정정됐는데 설명만 옛것이었다.
    #   화면 설명대로 손으로 검산하면 숫자가 안 맞아 「프로그램이 틀렸다」고 오해한다.
    "수수료율":     {"kind": "calc", "desc": "마켓수수료 ÷ 실결제금액(없으면 총주문금액)"},
    # 🔴 2026-08-13 판정 — 「+ 고객배송비 전액」이 아니다. 마켓이 배송비에서도 수수료를 뗀다
    #   (쿠팡 자기 정산 엑셀 455행 전수: 4,000→3,868 등 배송료 행 예외 0건, 실효 3.3%).
    #   실값을 주는 마켓은 그 값을, 없으면 고객배송비(옛 규약)로 떨어진다.
    "정산예정금(배송비포함)": {"kind": "calc",
                               "desc": "정산예정금액 + 배송비 정산액(마켓 실값·수수료 차감). "
                                       "실값을 안 주는 마켓은 고객배송비 전액(추정)"},
}
...
def lotteon_order_rows(since: _dt.datetime, until: _dt.datetime,
    ...
    #  🔴🔴 [2026-08-03] **양수만** 쓴다. 예전엔 `if v is not None` 이라 0 도 박았다.
    #    그때는 크롤표에 최근 것만 조금 있어서 티가 안 났는데, 정산 크롤 회차를 180일로
    #    넓히자(#704) 표에 **0원이 1,744건** 쌓이면서 결함이 그대로 드러났다:
    #    라이브 실측 2026-08-03 — real 인데 정산 0원인 행이 10건(전부 반품완료=정상)에서
    #    21건으로 늘고 그중 **배송완료가 5건**. 배송완료인데 정산 0원 = 팔았는데 한 푼도
    #    못 받았다는 뜻이라 마진이 「매입가 전액 손실」로 뒤집힌다. 게다가 라이브 조회가
    #    닿기만 하면 **122건(상품준비 92·출고지시 28·배송완료 2)** 이 더 그렇게 된다.
    #    크롤 0원이 「아직 미정산이라 0」인지 「취소돼서 진짜 0」인지 이 표만으론 못 가른다
    #    → 단정하지 않고 그대로 둔다(폴백·날조 금지). 취소의 진짜 0 은 zero_cancel 이 담당.
    #    ★정산 스윕(order_ingest.refresh_settlement_lotteon)과 **같은 규약**이다 —
    #      한쪽만 고치면 조회 경로에 따라 같은 주문이 다른 값을 갖는다.
    for r in rows:
...
CP_FEE_FACTOR = 0.8845        # 1 - 0.1155 (= 10.5% × 1.1). 옛 근사 상수 — 시험이 참조
...
#  🔴 상수 `CP_SHIP_FEE_FACTOR` 는 **근사값**이다. 실제 계산은 위 `cp_fee()` 정본을 쓴다
#    (버림 자리가 달라 값이 1원씩 갈릴 수 있다). 이 상수는 옛 시험이 참조해 남겨 둔다.
CP_SHIP_FEE_FACTOR = 0.967    # 1 - 0.033 (배송비 서비스이용율 3.0% + VAT 10%) — 근사
