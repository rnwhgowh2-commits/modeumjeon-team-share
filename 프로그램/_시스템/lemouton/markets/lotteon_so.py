...
왜 이 소스가 필요한가(2026-07-23 정답지 387건 대조로 확정):
...
셀러오피스 화면(soapi)이 이 셋의 유일한 원천 — 정답지가 보던 그 데이터다.
...
def upsert_rows(rows: list, *, session) -> dict:
    ...
    같은 배치 안 중복 키는 마지막 값 채택(pending 충돌 방지 — 다른 업서트 라우트들과 동형).
        ...
        if route in ("", "미확인", "확인 불가"):
            ...
            if ch:
                ...
                if by is not None:
                    ...
                    # 라벨은 '미확인' 한 낱말(사장님 확정 2026-07-25) — 왜 모르는지는 사유로.
                    r["판매경로"] = "미확인"
                    r["_판매경로사유"] = (f"셀러오피스 크롤에서 유입채널 {ch} 를 받았지만 "
                                       "제휴/직영 분류표에 없는 채널입니다.")
            elif route == "미확인":
                # 수집은 됐는데 원천에 채널값이 없다 = 봐도 없는 것. 라벨은 그대로 '미확인'
                # 이고 사유만 바뀐다 — 사장님이 볼 판단(믿어도 되나)은 어차피 같다.
                r["_판매경로사유"] = ("셀러오피스 크롤에는 이 주문이 있지만 유입채널 값이 "
                                   "없습니다(마켓이 안 줌).")

        # ── 철회 잔존 교정 — SO(셀러오피스 현재 화면)가 정답(정답지와 같은 원천) ──
