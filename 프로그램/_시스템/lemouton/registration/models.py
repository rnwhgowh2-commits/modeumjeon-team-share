...
class SearchFilter(Base):
    ...
    #: **이어서 걷기** — 다음 회차가 시작할 쪽. NULL = 처음부터.
    #: 🔴 왜 필요한가 — 한 회차 상한(`MAX_PAGES` 60쪽)은 소싱처를 몰아치지 않으려는
    #:   사람이 정한 안전선이다. 없앨 수 없다. 그런데 현대H몰 「나이키 신발」은
    #:   **456쪽 16,413개**라 60쪽으로는 13% 밖에 못 걷는다.
    #:   못 걷은 만큼 **팔 상품이 줄어든다** — 그건 매출 손실이다.
    #: ★ 그래서 상한은 지키되 **다음 회차가 이어서** 걷는다. 「더 있음」이면 창을
