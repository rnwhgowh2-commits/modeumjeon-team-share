...
ITEM_KEYS = (
    ...
    "banned_words",  # 금지어
    "tags",          # 태그
    # ── [2026-08-12] 사장님 엑셀 「마켓별 상품등록 정보」 대조로 추가한 3항목 ──
    #   🔴 마켓 등록 API 에는 **원래 있던** 칸인데 우리 정책에만 없었다.
    #      비워 두면 마켓이 거부하거나 제멋대로 기본값이 박힌다.
    #   사장님 지시 — 「거의 유사한 것들은 기본설정으로 해두면 될듯해」
    #      (인증대상아님 / 판매기간 무제한 or 가장 길게 / 전체연령 / 새상품 …)
    "listing",       # 등록 기본값 (과세·상품상태·판매기간·미성년자·제조사)
    "price_compare", # 가격비교 노출
    "ids",           # 모델번호·바코드
)
...
ITEM_LABELS = {
    ...
    "listing": "등록 기본값", "price_compare": "가격비교 노출", "ids": "모델번호·바코드",
}
