...
#:              `요청.sellerProductName` = "(Body) 등록상품명  발주서에 사용되는 상품명
#:              최대 길이 : 100 자" — **등록 필드 자체의 선언**이다.
...
#: smartstore : [2026-07-23 리뷰 I4 정정] 등록 API 의 `요청.name` 에는 길이 선언이
#:   **없다**(marketplace_api_map.json:118182 · 129482 · 147447 = "상품명 (string) [필수]").
#:   전에 근거로 쓴 :103531 은 **다른 필드**(`요청.promotionText` 추가 문구) 설명의
#:   "추가 문구를 포함한 노출 상품명은 최대 100자까지" 문장이었다 — 정황은 강하지만
#:   상품명 필드의 스펙이 아니다. 롯데온에 적용한 잣대(등록 API 에 길이 없음 = 확인 불가)를
#:   스스에만 느슨하게 쓰면 그 자체가 모순이라, 같은 기준으로 「확인 불가」에 둔다.
#:   ※ 승격 조건: 커머스API센터 상품 등록 문서에서 `name` 의 길이 선언을 확보해
#:     지도에 되채운 뒤(consult-market-map 3-②) NAME_MAX_LEN 으로 옮긴다.
NAME_MAX_UNKNOWN = {
    # ★ [2026-07-24 2차 리뷰 ④] 「안 자릅니다」로 끝내지 않는다 — 안 자른 결과
    #   **마켓이 길이 초과로 거부할 수 있다**는 사실까지 말해야 정직한 안내다.
    'auction': ('옥션 상품명 상한은 byte 기준(검색용+프로모션용 합계)이라 글자수로 '
                '바꿀 수 없습니다 — 지도에 인코딩이 없어 「확인 불가」로 둡니다. '
                '잘못 자르면 잘린 채로 팔리므로 자르지 않고 그대로 보냅니다. '
                '이름이 길면 옥션이 길이 초과로 등록을 거부할 수 있습니다.'),
    'gmarket': ('G마켓 상품명 상한은 byte 기준(검색용+프로모션용 합계)이라 글자수로 '
                '바꿀 수 없습니다 — 지도에 인코딩이 없어 「확인 불가」로 둡니다. '
                '잘못 자르면 잘린 채로 팔리므로 자르지 않고 그대로 보냅니다. '
                '이름이 길면 G마켓이 길이 초과로 등록을 거부할 수 있습니다.'),
    'lotteon': ('롯데온 상품등록 API 스펙에 상품명 길이가 적혀 있지 않습니다 — '
                '「확인 불가」로 두고 자르지 않습니다. 이름이 길면 롯데온이 길이 '
                '초과로 등록을 거부할 수 있습니다.'),
    'smartstore': ('스마트스토어 상품등록 API 스펙에 상품명 길이가 적혀 있지 않습니다 '
                   '(100자는 「추가 문구」 필드 설명에 나온 정황일 뿐입니다) — '
                   '「확인 불가」로 두고 자르지 않습니다. 이름이 길면 스마트스토어가 '
