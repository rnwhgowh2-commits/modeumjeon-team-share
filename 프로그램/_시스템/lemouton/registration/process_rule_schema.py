...
SCHEMAS: dict = {
    ...
    "price": ItemSchema(
        ...
        fields=(
            ...
            # [2026-08-13] 사장님 확정 — 「스스 20% 는 실제 할인이 아니라 고객에게 싸게
            #   보이려고 걸어 둔 값」. 그런 할인은 **판매자가 100% 부담**이라 마진을
            #   할인가 기준으로 재야 한다. 반대로 마켓이 같이 부담하는 할인(롯데온 등)은
            #   그 몫이 우리 수입이라 판매가를 올릴 이유가 없다.
            #   🔴 기본값은 「판매자」다 — 모르면 보수적으로(판매가를 넉넉히) 잡는다.
            #     「마켓」으로 잘못 두면 판매가를 안 올려 그대로 적자가 된다.
            _F("discount_burden", "이 할인 누가 부담하나", "choice", default="seller",
               choices=("seller", "market", "split"),
               hint="판매자 = 우리가 다 부담(고객에게 싸게 보이려는 값) · "
                    "마켓 = 마켓이 다 부담 · 반반 = 아래 비율만큼만 우리가 부담"),
            _F("discount_burden_pct", "우리가 부담하는 몫", "int", default=None, unit="%",
               hint="「반반」일 때만 씁니다. 예: 50 = 할인액의 절반을 우리가 냄"),
    ...
    "price_compare": ItemSchema(
        ...
        fields=(
            ...
            _F("fee_add_pct", "가격비교 수수료 가산", "int", default=2, unit="%",
               hint="🔴 계산에 **일부러** 안 더합니다 — 롯데온 18%·옥션 15%·"
                    "G마켓 15%·스스 6% 는 이 2% 가 **이미 포함**된 값이라 또 더하면 "
                    "이중으로 빠집니다(판매가가 그만큼 비싸져 안 팔립니다). "
                    "11번가만 포함 여부를 아직 확인하지 못했습니다 — 확인되면 "
                    "수수료율 숫자 자체를 고칩니다. 이 칸은 기록용입니다"),
        )),
    "ids": ItemSchema(
