...
SEED: dict[str, dict] = {
    ...
    # 🔴 [2026-08-13 사장님 확정] 18.0 → **13.0**. 18% 는 어디서도 뒷받침되지
    #   않았고, 실정산 엑셀 86행 오차0 으로 검증된 `lotteon_settlement` 도
    #   상품 요율 13% 를 쓴다. 18% 로 두면 판매가가 **6.5% 비싸게** 나갔다.
    'lotteon':    {'base_pct': 13.0,  'alt_label': '', 'alt_pct': None},
...
OLD_SEED_FIX = {
    'lotteon':  {'old_base': 18.0, 'new_base': 13.0},
...
NOTES: dict[str, str] = {
    'lotteon': '판매수수료 13% 입니다 — 제휴 경유로 들어온 주문은 상품가 2% 가 '
               '**따로 더** 빠집니다(직영 주문은 안 빠집니다).',
...
RATE_EVIDENCE: dict[str, dict] = {
    ...
    'lotteon': {
        'kind': 'measured',
        'source': '2026-08-13 사장님 확정 13% — 실정산 엑셀 86행 오차0 으로 검증된 '
                  'lotteon_settlement.compute_settlement 의 상품 요율(0.13)과 같다',
        'note': '🔴 **제휴 경유 주문은 상품가 2% 가 더 붙는다**(판매경로=제휴일 때만 · '
                'order_export 판매경로 3상태). 판매가는 13% 로 내므로, 제휴로 들어온 '
                '주문은 목표보다 2%p 적게 남는다 — 실제 마진은 정산 실값으로 잡힌다.',
    },
    'eleven11': {
        'kind': 'measured',
...
AFFILIATE_IN_BASE: dict[str, bool | None] = {
    ...
    'lotteon': False,      # 13% = 판매수수료. 제휴 2% 는 제휴 경유 주문에만 따로
