...
SELL_COLUMNS = [
    ...
    "옵션추가금", "상품금액", "총주문금액",
    # order_export._finalize_rows 가 만드는 매출의 단일 원천(2026-08-28 확정: 할인
    # 무관 정가+배송비). 이 칸을 안 실으면 pipeline._CARRY_FIELDS 조인이 0 으로
    # 밀어 넣어, 마진탭/엑셀이 실결제(할인 반영) 기준으로 조용히 되돌아간다
    # (실측: 롯데온·11번가·스마트스토어 매출이 할인만큼 과소, 2026-09-06).
    "_매출기준액",
...
def _rows_to_df(rows: list) -> pd.DataFrame:
    ...
    for r in rows:
        ...
        out.append({
            ...
            "_매출기준액": _to_int_or_blank(r.get("_매출기준액")) or 0,
            "정산예상금액_배송비포함": settle,
            "마켓수수료": r.get("마켓수수료", ""),
            "수수료율": r.get("수수료율", ""),
