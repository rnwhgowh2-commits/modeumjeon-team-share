...
#     🔴 롯데온 13.0 → 15.0 (2026-08-13) — 판매수수료 13 + 유입(제휴) 2.
#        제휴를 늘 켜므로 실제 정산에서 떼이는 값이 15% 다. 13 으로 두면 정상인데도
#        2%p 어긋난 것처럼 보인다(롯데ON 직접 유입 주문만 13%).
#     🔴 11번가 11.0 → 13.0 (2026-08-13 사장님 확인) — 「11%(1년 이내 8%)이고
#        **가격비교 노출 2% 는 미포함**」. 그 기능을 늘 켜므로 실제 정산에서 떼이는
#        값은 13% 다. 11 로 두면 정산율이 정상인데도 2%p 어긋난 것처럼 보인다.
#     ★ 이 표는 `pricing/fee_defaults.SEED`(판매가를 정하는 값)와 **같은 뜻**이어야
#       한다. 한쪽만 고치면 「정산이 이상하다」는 거짓 경고가 뜨거나, 진짜 이상을 놓친다.
_EXPECT_FEE_PCT = {"coupang": 11.55, "smartstore": 6.0, "lotteon": 15.0,
