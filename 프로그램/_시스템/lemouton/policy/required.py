...
WIRING: dict[str, tuple[str, str]] = {
    ...
    # 🔴 「저장만」이지만 **이미지가 안 나간다는 뜻이 아니다.** 사진은 나간다 —
    #   안 나가는 것은 「어떤 사진을 어떤 순서로」라는 **정책 규칙** 쪽이다.
    #   앞머리를 「안 나갑니다」로 읽히게 두면 사장님이 사진이 빠진 줄 아신다.
    'images': (STORED_ONLY, '사진 자체는 마켓으로 나갑니다 — 다만 이 정책의 '
                            '이미지 **규칙**(어떤 걸 대표로, 몇 장, 어떤 순서)은 아직 '
                            '읽지 않습니다. 초안은 이미지를 **옵션에서 다시 모아** '
                            '붙입니다 (send/as_draft.py 의 _option_images).'),
...
#: 🔴 [2026-08-13] 「미성년자 구매」가 **어느 마켓에도 안 나가고** 있었다 —
#:   정책에 칸은 있는데 읽는 코드가 0곳, 초안 칸은 기본값(전연령) 그대로,
#:   쿠팡은 `adultOnly='EVERYONE'` 상수. 「19세 이상만」을 고르셔도 무시됐다.
#:   지금 **스스·쿠팡 두 곳만** 이었다 — 나머지는 조립기 spec 에 칸 자체가 없다.
#:   ⚠️ 「등록 기본값」 묶음의 나머지 칸(과세·상품상태·판매기간·제조사)은 여전히
#:     상수가 박혀 있다. 그래서 상태는 **저장만**을 유지한다 — 일부만 되는 것을
#:     「나감」으로 적으면 나머지가 조용히 묻힌다.
WIRING['listing'] = (
    STORED_ONLY,
    '「미성년자 구매」는 [2026-08-13] 부터 **스마트스토어·쿠팡 두 곳에 실제로 '
    '나갑니다** — 「19세 이상만」을 고르시면 그대로 등록됩니다. '
    '🔴 11번가·옥션·G마켓·롯데온은 아직 보낼 자리를 안 만들었습니다. '
    '같은 묶음의 과세구분·상품상태·판매기간·제조사는 아직 어느 마켓에도 정책값이 '
    '안 나갑니다 — 등록 코드에 값이 박혀 있습니다.')
...
WIRING['discount'] = (
    STORED_ONLY,
    '즉시할인·쿠폰을 **마켓으로 보내는** 코드는 아직 0곳입니다 — 할인은 사장님이 '
    '마켓 관리자 화면에서 직접 겁니다. '
    '다만 [2026-08-13] 부터 이 값이 **판매가 계산에는 들어갑니다** — 판매자가 '
