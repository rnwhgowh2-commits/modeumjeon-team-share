...
#:   나머지 다섯이 조용히 묻힌다.
def _discount_wiring_note() -> str:
    """🔴 목록을 여기 손으로 박아 두면 `discount.SUPPORTED` 가 늘 때마다 낡는다
    (실제로 쿠팡 하나였다가 옥션·G마켓이 추가되며 한 번 낡았다). 정본에서 읽는다.
    """
    from lemouton.policy.discount import SUPPORTED, market_label
    from lemouton.policy.fields import MARKET_KEYS
    나감 = '·'.join(market_label(m) for m in SUPPORTED)
    아직 = '·'.join(market_label(m) for m in MARKET_KEYS if m not in SUPPORTED)
    return (
        f'즉시할인은 **{나감}에만** 실제로 나갑니다 — 즉시할인쿠폰을 만들어 옵션에 '
        '붙입니다(⏰ 쿠팡은 다음날 0시부터 적용). '
        f'🔴 {아직}은 아직 보낼 자리를 못 찾았습니다 — 그쪽 할인은 마켓 관리자 '
        '화면에서 직접 거셔야 합니다. '
        '다만 여기 적으신 값은 **여섯 마켓 모두 판매가 계산에 들어갑니다** — '
        '판매자가 부담하는 할인만큼 판매가를 올려 잡아, 할인 뒤에도 목표 마진이 '
        '남게 합니다. 그래서 이 칸을 비워 두면 마켓에 실제로 걸린 할인만큼 그대로 '
        '손해가 납니다.')


WIRING['discount'] = (STORED_ONLY, _discount_wiring_note())
