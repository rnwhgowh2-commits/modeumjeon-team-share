# -*- coding: utf-8 -*-
"""「벌」 — 상품 하나가 마켓에 나가는 **한 벌**. 정책이 붙는 단위.

■ 벌이란 (2026-08-02 사장님 확정)
  「한 상품에 정책 여러 개」 = 같은 상품을 정책마다 다르게 가공해 **같은 마켓에 각각** 올리기.
  그 「한 벌」은 이미 집에 있다 — **구성(ProductSet)** 이다.

  🔴 벌을 가르는 건 **정책**이지 「단품/세트」가 아니다.
     단품+단품 · 세트+세트 · 단품+세트 무엇이든 된다. 그래서 화면에서 벌은
     **정책 이름으로 부르고**, 구성 이름은 있으면 옆에 붙는 보조 정보다.

■ 왜 벌 하나 = 구성 하나인가
  `SetChannel` 에 `UNIQUE(set_id, market, account_key)` 가 걸려 있다. 한 구성은
  한 마켓·한 계정에 상품을 **하나만** 올릴 수 있다. 그러니 마켓에 두 벌을 올리려면
  구성이 둘이어야 한다. 「단품, 단품」이면 구성을 2개 만든다.

■ 재고
  나누지 않는다(사장님 확정 「데이터 연동」). `reconcile.market_targets_for(sku)` 가
  그 옵션이 걸린 **모든** 구성·마켓을 돌려주므로, 벌이 둘이면 둘 다에 같은 재고가 나가고
