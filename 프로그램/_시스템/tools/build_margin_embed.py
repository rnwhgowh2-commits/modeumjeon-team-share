...
SEAMS: list[tuple[str, str, int]] = [
    ...
    # 3) 업로드 엔드포인트: 매입(더망고)만 업로드. 매출은 마켓 API 자동 조회라 업로드 없음
    #    (2026-09 전 마켓 API 연동 완료로 매출 보조 엑셀 업로드 폐지 — sell 입력칸은
    #    씨앗 12) 에서 이미 비활성화돼 이 분기가 실제로 갈 일이 없다).
    (
        "    const res  = await fetch('/api/upload', { method: 'POST', body: fd });",
        "    const _mUploadUrl = '/api/margin/upload';  /* [모음전] 매입(더망고)만 업로드 — 매출은 마켓 API 자동조회 */\n"
    ...
    # 12) [모음전 신규 씨앗] 매출 = 마켓 API 자동 조회 → 매출 엑셀 업로드칸 제거.
    #     원본은 매입·매출 두 업로드칸이 있으나, 모음전은 SALES 를
    #     분석 시점에 판매처 마켓 API 에서 자동 조회한다(사용자가 매출 엑셀을 올리지
    ...
    (
        ...
        "      <div class=\"upload-sub\">업로드 불필요 — 분석 시작 시 판매처 API에서 매출을 불러옵니다</div>\n"
    ...
    #     ★2026-08-27/28 사장님 재확정 — 매출 = 정가(단가×수량+옵션) + 배송비, 할인은
    #        판매자부담이든 마켓부담이든 **전혀 안 뺀다**(2026-08-13엔 판매자부담만
    #        뺐다가 다시 확정 — PR#1135). 값은 주문내역(order_export)이 `_매출기준액`
    #        한 칸으로 만들어 둔다 — 화면은 **받아 쓰기만** 한다(두 곳이 각자 계산하면
    #        규약 바뀐 날 조용히 갈린다).
    #        🔴 2026-09-06 — sell_source._rows_to_df 가 이 칸을 안 실어 날라 마진탭·
    #        엑셀이 폴백(옛 실결제 기준)으로 조용히 되돌아가 있었다(롯데온·11번가·
    #        스마트스토어 매출이 할인만큼 과소, 쿠팡·G마켓은 우연히 일치해 안 들킴).
    #        원인은 sell_source.py 를 고쳐 다시 실었고, 여기 폴백도 실결제(할인 반영)
    #        가 아니라 정가(할인 무관)를 향하게 고쳐 이중 안전판을 둔다.
    (
        "function saleAmt(r) {",
        "function saleAmt(r) {\n"
        "  if (!r) return 0;  /* [모음전 매출기준] r 접근 전 가드 */\n"
        "  var _sale = Number(r['_매출기준액']);  /* [모음전 매출기준] 정가+배송비, 할인 무관(2026-08-28) */\n"
        "  if (isFinite(_sale) && _sale > 0) return _sale;  /* [모음전 매출기준] 단일 원천 */\n"
        "  var _gross = Number(r['총주문금액']) || Number(r['상품금액']) || 0;  /* [모음전 매출기준] 폴백도 정가(할인 무관) — 실결제 아님 */\n"
        "  if (_gross > 0) return _gross + (Number(r['배송비']) || 0);  /* [모음전 매출기준] */",
        1,
    ),
    ...
    #     [2026-09-06] 부제 문구도 2026-08-28 확정(할인 무관)에 맞춘다.
    (
        "     + _sumCard('총매출','판매가', fmt(매출)+'원')",
        "     + _sumCard('총매출','정가+배송비(할인 무관)', fmt(매출)+'원')  /* [모음전 매출기준] 표기 */",
        1,
    ),
    ...
    (
        ...
        "  var cols = ['제외','비대량등록','더망고주문상태 (사용자 연동)','마켓주문상태 (오픈 마켓 연동)','국내송장번호','판매처_주문상태','판매처_송장입력','판매처_택배사','주문일','_소싱처','마켓','마켓주문번호','수령인명','브랜드','상품명','옵션_매출','단가','수량_매출','실결제금액','배송비','정산예상금액','구매가격','순마진','마진율','판매경로','소싱처확인결과','간단메모','_추가메모'];  /* [모음전] 전체내역 열 구성 — 사장님 지정 순서 2026-07-24, 제외·비대량등록·간단메모 추가 2026-07-25 */",
        1,
    ),
    (
        "    '소싱처확인결과':'소싱처 주문상태',\n    '_추가메모':'추가메모'\n  };",
    ...
    (
        ...
        "  var usedCols = cols.filter(function(c){ if(c==='소싱처확인결과' || c==='_추가메모' || c==='제외' || c==='비대량등록' || c==='간단메모' || c==='판매처_택배사') return true; return rows.some(function(r){ return r[c]!=null && r[c]!==''; }); });  /* [모음전] 제외·비대량등록·간단메모 항상 표시 */",
        1,
    ),
    ...
    #     🔴 [2026-09-06] 이 함수(recomputeRow)는 `saleAmt()`와 달리 건드리지 않는다 —
    #        margin_flags.py 가 이 함수를 그대로 이식해 「원본과 같은 답」을 대조 시험한다
    #        (tests/orders/test_margin_flags.py). 여기 규칙을 saleAmt() 처럼 바꾸려면
    #        margin_flags.recompute_row 도 같이 바꿔야 하며, 그건 「주문내역 탭 배지
    #        판정」이라는 별도 화면의 정책 결정이라 이번 매출액 버그 수정 범위 밖이다.
    #        실제 버그(마켓별 매출 과소)는 saleAmt()/카드 집계 쪽이었고, 여긴 셀 편집
    #        시에만 도는 좁은 경로라 sell_source 원천 수정만으로 이미 옳게 동작한다
    #        (_sale0 이 이제 정가+배송비 그대로 오므로 _sellerDc 가 0 이 되어 그대로 맞다).
    (
    ...
    #     [판매처] 송장번호(판매처_송장입력) → '판매처_택배사'(ESM TakbaeName·pipeline 이 부착).
    ...
    (
        ...
        "        return;  /* [모음전] 택배사 */\n"
        "      }  /* [모음전] 택배사 */\n"
        "      if(c==='판매처_택배사'){  /* [모음전] [판매처] 택배사 별도 칼럼 (사장님 요청 2026-07-25) */\n"
        "        var _pc=String(r['판매처_택배사']||'').trim();  /* [모음전] 없으면 빈칸 — 이름 날조 금지 */\n"
    ...
    # 36) [모음전 2026-09-06 사장님 확정 — 주문상태는 API 명명어만 쓴다] 롯데온 정산
    #     백필(SettleProduct) odTypCd=10 은 API 문서 그대로 "주문"이다(라벨을 지어내지
    #     않는다 — marketplace_api_map.json 확인). 이 백필은 배송중/구매확정 같은 배송
    #     단계를 아예 안 주므로, 위 26번 씨앗의 키워드 어디에도 안 걸려 국내배송중
    #     + 구매가격 있는 정상 매출 116건이 통째로 "기타" 카드로 샜다(2026-09-06
    #     라이브 실측 — 116건 전부 롯데ON·판매처_주문상태="주문"). 정확히 일치(===)할
    #     때만 잡는다 — 다른 마켓의 서술형 상태문구(예: "OO 주문 접수")와 안 겹치게.
    (
        "        sm.indexOf('구매확정') >= 0 || sm.indexOf('수취완료') >= 0 || sm.indexOf('배송완료') >= 0 || sm.indexOf('확정') >= 0 || sm.indexOf('배송') >= 0 || sm.indexOf('출고지시') >= 0  /* [모음전] 롯데온 출고지시 — 기타로 새던 53건 */",
        "        sm.indexOf('구매확정') >= 0 || sm.indexOf('수취완료') >= 0 || sm.indexOf('배송완료') >= 0 || sm.indexOf('확정') >= 0 || sm.indexOf('배송') >= 0 || sm.indexOf('출고지시') >= 0 || sm === '주문'  /* [모음전 2026-09-06] 롯데온 정산백필(odTypCd=10, API 원문 그대로 \"주문\") — 기타로 새던 116건. 정확히 일치할 때만(다른 마켓의 서술형 상태문구와 겹치지 않게) */",
        2,
    ),
...
# 27) 판매경로 칸 — '미확인'을 회색으로 떼기 (2026-07-25 정답지 대조).
...
# ── [모음전 신규 씨앗] 「검산 요약」 + 실마켓(API) 미매칭 카드 + 화면 라벨 → '실마켓(API)' 개명
#    (사장님 확정 2026-07-25) ────────────────────────────────────────────────
...
# 구 라벨 문구 → '실마켓(API)'.
#   ★아래 목록은 원본(벤더 단독앱)의 화면 표시 라벨만 겨눈 씨앗이다 — 원본 파일 자체를
#     이 저장소가 갖고 있지 않으므로 old 쪽 문자열은 원본 그대로 두어야 매칭이 유지된다.
#     데이터 키(판매처_* 필드명, 데이터출처 값 '판매처만'·'더망고+판매처')는 위쪽 cols/
#     colLabels 씨앗에서 이미 판매처_* 로 통일했다(matcher.py 출력과 맞춤, 2026-09).
...
# ── [모음전 2026-09-06] 취소요청 클레임 뒤 실제 진행된 주문은 진행중 카드로 안 감 ──
#  사장님 신고: 취소요청('26-09-04) 클레임이 걸렸는데도 주문 자체가 그 뒤 배송완료
#  ('26-09-06)까지 진행됐다(취소가 안 받아들여짐) — 그런데도 판매처 주문상태 텍스트가
#  여전히 "취소요청"이라 진행중 카드로 셌다. 서버가 이력(status_at)으로 이미 재판정해
#  붙여 준 r['정산여부']를 우선해 무시한다(lemouton/margin/card_counts.py 와 동일 판단,
#  단일 원천). 원본에 이 두 줄이 2회 나온다(_getGroupCardBreakdown·
#  _getRowsByCardFilter_internal 각 1회) — 둘 다 바꿔야 화면과 서버 집계가 안 갈린다.
SEAMS.append((
    "    var smC = smCat(sm);\n"
    "    var mg = String(r['더망고주문상태 (사용자 연동)'] || '');",
    "    var smC = smCat(sm);\n"
    "    /* 🔴 [2026-09-06 사장님 확정] 취소요청 등 클레임 뒤에도 주문 자체가 실제로\n"
    "       배송·구매확정까지 진행될 수 있다(클레임 미반영 — 실사례: 취소요청 '26-09-04\n"
    "       → 배송완료 '26-09-06). 서버가 이력(status_at)으로 이미 재판정해 둔 '정산여부'가\n"
    "       있으면 그걸 우선한다 — 여기서 텍스트만 보고 또 진행중으로 세지 않는다\n"
    "       (card_counts.py 와 동일 판단, 단일 원천). */\n"
    "    if (smC === 'in_progress' && r['정산여부'] && r['정산여부'] !== '진행중') smC = 'normal';\n"
    "    var mg = String(r['더망고주문상태 (사용자 연동)'] || '');",
    2,
))
