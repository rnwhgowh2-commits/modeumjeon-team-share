...
SEAMS: list[tuple[str, str, int]] = [
    ...
    # 36) [모음전 2026-09-06] 롯데온 "매출확정"(정산 백필 SettleProduct 의 정상매출
    #     구분값, 옛 이름 "주문") → 정상/완료. 이 백필은 배송중/구매확정 같은 배송
    #     단계를 아예 안 주므로, 위 26번 씨앗의 키워드 어디에도 안 걸려 국내배송중
    #     + 구매가격 있는 정상 매출 116건이 통째로 "기타" 카드로 샜다(2026-09-06
    #     라이브 실측 — 116건 전부 롯데ON·판매처_주문상태="주문"). 26번과 같은 자리에
    #     이어 붙인다 — shared/platforms/lotteon/settle_orders.py 의 라벨 개명과 짝.
    (
        "        sm.indexOf('구매확정') >= 0 || sm.indexOf('수취완료') >= 0 || sm.indexOf('배송완료') >= 0 || sm.indexOf('확정') >= 0 || sm.indexOf('배송') >= 0 || sm.indexOf('출고지시') >= 0  /* [모음전] 롯데온 출고지시 — 기타로 새던 53건 */",
        "        sm.indexOf('구매확정') >= 0 || sm.indexOf('수취완료') >= 0 || sm.indexOf('배송완료') >= 0 || sm.indexOf('확정') >= 0 || sm.indexOf('배송') >= 0 || sm.indexOf('출고지시') >= 0 || sm.indexOf('매출확정') >= 0  /* [모음전 2026-09-06] 롯데온 정산백필 매출확정 — 기타로 새던 116건 */",
        2,
    ),
