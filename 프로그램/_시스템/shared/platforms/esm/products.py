...
def build_esm_register_payload(
    *,
    ...
    # ★ [2026-08-26] 즉시할인(판매자할인) — 지도 실측 `addtionalInfo.sellerDiscount`.
    #   별도 API 가 아니라 **등록 요청 안에** 있다.
    #   🔴 조각은 `policy/discount.esm_seller_discount` 가 만든다 — 여기서 다시
    #     만들면 옥션(iac)/G마켓(gmkt) 키를 헷갈릴 위험이 두 곳으로 늘어난다.
    #   🔴 안 정했으면 **칸 자체를 안 넣는다.** 지금까지 안 보내던 칸이라, 기본값을
    #     지어 넣으면 사장님이 정한 적 없는 할인이 걸린다(금전 직결).
    if seller_discount:
        body["addtionalInfo"]["sellerDiscount"] = seller_discount
