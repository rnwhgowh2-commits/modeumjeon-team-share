...
# 🔴 [2026-09-06 사장님 확정] 주문상태 값은 **마켓 API 가 쓰는 명명어만** 쓴다 — 우리가
#   지어낸 말을 섞지 않는다. "10"은 marketplace_api_map.json 이 확인한 롯데온 API 문서의
#   실제 표기(odTypCd "주문유형[10주문/20취소/30교환/40반품]") 그대로 "주문"이다.
#   이 값이 어떤 배송 단계 키워드와도 안 걸려 마진계산기 카드 분류기에서 정상 매출
#   116건이 "기타"로 새고, 주문상태 이력에도 "취소요청 → 주문"처럼 낯설게 보이는
#   문제가 있었지만 — 그건 이 라벨이 틀려서가 아니라 **이 값을 못 알아보는 쪽**(분류기)의
#   문제였다. 라벨은 그대로 두고 분류기 쪽을 고친다
#   (tools/build_margin_embed.py 의 "출고지시" 씨앗과 같은 자리 참고).
