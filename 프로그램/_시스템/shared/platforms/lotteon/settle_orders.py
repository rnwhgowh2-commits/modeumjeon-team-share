...
# 🔴 [2026-09-06] "10"을 예전엔 "주문"으로 적었다 — 이 API는 배송중/구매확정 같은
#   배송 단계를 아예 안 주고 "취소 아닌 정상 매출"이라는 정산 구분만 준다. "주문"이라는
#   글자가 다른 화면(주문상태 이력·마진계산기 카드 분류)에서 "이제 막 새 주문이 생겼다"
#   처럼 읽혀 — 클레임 이력에 "취소요청 → 주문"으로 떠 사장님이 되돌아간 걸로 오인했고
#   (재확인 결과 취소가 아니라 정상 정산으로 끝난 것), 마진계산기에서는 이 값이 어떤
#   배송 단계 키워드와도 안 걸려 정상 매출 116건이 통째로 "기타" 카드로 샜다
#   (프로그램/_시스템/tools/build_margin_embed.py 의 "출고지시" 씨앗과 같은 자리에 등록).
_TYPE = {"10": "매출확정", "20": "취소완료", "30": "교환완료", "40": "반품완료"}
