...
| **현대H몰** | 단품=내장 `__NEXT_DATA__` / 모음전(2축)=`item-stockcount` API(서버사이드) | 단품: `itemPtc.stockList[].stockCount`(0=품절·N=실수량) / 모음전: 색별 `item-stockcount` 프로브 → `sellGbcd`("00"=판매·그 외=품절, `stockCount=1`은 품절센티넬) | `last_stock`·`current_stock` | `crawlers/hmall.py`(`fetch_combo_persize_options`) |
...
- **현대H몰**: 단품=`stockCount`(0=품절·N=실수량) / 모음전(색×사이즈)=색별 `item-stockcount` API 프로브 → `sellGbcd`("00"=판매중·그 외="11"=품절). ⚠️품절 사이즈도 `stockCount=1` 센티넬을 주므로 **반드시 `sellGbcd` 우선**(stockCount 단독 판정=거짓 '1개 있음'). 공개 API라 서버사이드 수집(확장 불요)
