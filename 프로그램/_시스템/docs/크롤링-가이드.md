...
| S14 | 전 소싱처 | 기타(엔진) | ⏳ | 전체크롤 미완료 시 전 매트릭스가 가짜 '재고있음' | 크롤 시작 하드리셋(NULL·pending)인데 엔진/스케줄 중단으로 완료 안 됨→pending(null재고)을 표시층이 '있음' 해석 | 크롤 미완료면 pending 명시·'있음' 둔갑 금지 | — |
...
| P9 | 무신사 등 | 백엔드 | ⏳ | 혜택 금액 0으로 저장 | 크롤이 회원혜택(등급적립·머니) OFF 금액0 저장 + 부분 스냅샷이 템플릿 가림 | 혜택 금액0 의심·게이트 | — |
...
| P13 | URL 저장 | 백엔드 | ⏳ | 무효 SKU를 거부 건수 보고 없이 조용히 skip | `_sync_option_links`가 rejected 무보고(재고매핑은 보고) | URL 저장도 rejected 건수 보고 | — |
| P14 | 매트릭스 | 프론트 | ⏳ | '재계산▶' 버튼이 저장 0인 stub | 버튼이 실제 재계산·저장 안 함 | 재계산 실제 구현 또는 버튼 숨김 | — |
| P15 | 가격설정 일괄저장 | 백엔드 | ⏳ | 허용목록 밖 필드를 조용히 무시 | `api_pricing.py` `price-config/bulk`(`bulk_set_price_config`) 허용목록 밖 무음 skip | 거부 필드 보고 | — |
...
| P23 | 단일옵션 | 백엔드 | ⏳ | 레거시 단일옵션 price-calc가 크롤가 없으면 95000 가짜 매입가 표시 | `get_price_breakdown`(`api_pricing.py:1630`)에 `or 95000` 폴백 잔존 — 메인 경로(`compute_market_price`)만 고쳐 단일 진실 원천 분열(§4 폴백가 금지 위반) | 이 엔드포인트도 폴백 제거·`compute_market_price` 통일 | — |
...
| O5 | 크롤 위젯 | 프론트 | ⏳ | 가격 없어도 '성공 ✓' 표기 | 위젯이 price=null을 성공으로 표기(honesty 버그) | 유효가격 없으면 실패 표면화 | — |
...
| O16 | 서버 refetch | 백엔드 | ⏳ | 단건 재크롤(refetch)이 SSG를 SSF 크롤러로 보내 오파싱/실패 | `_detect_site_from_url`(`api_pricing.py:1656`)이 `'ssg.com' in u → 'ssf'`로 묶음(`build_crawlers`에 'ssg' 키는 실재→URL 매퍼만 고치면 됨) | ssg.com → 'ssg'로 분기 | — |

> **알려진 구조적 결함(보충)**: SSG/SSF 혜택이 라이브 확장크롤로 갱신 안 됨(P3) — `toItemBG`·`save_crawl_result`(api_pricing.py:1115)가 **price·stock만 저장**하고 `dynamic_benefits_json`(ssg_money 등) 미저장. + 서버 단일 재크롤 `refetch`가 SSG를 SSF로 오분류(O16) — `_detect_site_from_url`(api_pricing.py:1656)이 `ssg.com→'ssf'`로 묶음(`build_crawlers`에 **'ssg' 키는 실재**하므로 URL 매퍼 수정이면 됨).
