...
- **선반영 판별 게이트** — 소싱처가 이미 경유 할인을 붙여 노출하면 `naver_via_preapplied=True` 로 표시하고 **주입하지 않는다**(재차감=이중차감 방지). 경유면 캐시백 off 는 엔진 기존 규칙이 자동 처리.
  - **현대H몰**: `item-ptc` 의 `tcDcInf`(`tcCdNm`='네이버가격비교'·`dcRate` 8·`tcDcAmt` 14,730)로 판별. 🔴 **raw HTML 엔 없다**(할인내역이 JS 렌더) — HTML 문자열 검색 구현은 실패한다.
  - **롯데온**: `favorBox` 의 「제휴할인」 항목 → preapplied.
...
**함정**
- 🔴 **드롭다운을 경로로 착각** — 롯데아이몰 `div.his` 안 `div.hislayer` 는 그 단계의 **형제 카테고리 목록**(여성브랜드의류·패션잡화…)이라 경로가 아니다. SSG 와 같은 구조 → **직계 자식만**(`recursive=False`) 읽는다. fixture 로 못 박음.
