...
|---|---|---|---|
| **르무통 공홈** | `meta[og:image]`(대표) + `.xans-product-addimage img`(추가 5) = **6장** | `#proDetail div.inner div.cont` (img 18장) | `crawlers/lemouton.py::_parse_image_urls`·`_parse_detail_html` (+ `lemouton_playwright.py` 동일 셀렉터) |
...
**정리 규칙(공통)** — `base.build_image_urls`(절대화·순서유지 중복제거·아이콘/로고/1px/스킨호스트 배제·확장자 확증) · `base.sanitize_detail_html`(script·iframe·on* 핸들러·HTML 주석 제거, img/a 절대화, 알맹이 없으면 빈 문자열).
