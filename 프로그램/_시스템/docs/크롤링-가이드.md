...
| **신규 등록 URL 확장크롤 '완료'인데 셀 '크롤링 미실시'** | 등록(`api_add_source_url`)은 `BundleSourceUrl`만 만들고 `SourceProduct`는 안 만듦 → 확장 저장(`save_crawl_result`)이 기존 행만 갱신해 신규 URL 결과를 `not_found`로 조용히 드롭. **수정(2026-06-26)**: `save_crawl_result`가 등록된 URL이면 `source_key`로 `SourceProduct` 생성 후 저장(서버사이드 전체크롤과 대칭). 무결성 §4 #4(누락 무경고) 복구. |
