...
- **롯데아이몰** (`_site_for=='lotteimall'` 분기 — `api_benefits.py:1124~` 힌트): 표면노출가(카드 미적용 할인가) → −L.POINT 적립 → −{카드} 청구할인(크롤 정액) / 무이자 할부 / **엔진 주입(카탈로그)**: OK캐시백 2.5%(`base_ratio` 0.9)·리뷰적립(텍스트) 100원 + 현대카드 2.73% 플로어
  - **다운로드 쿠폰 = 「할인쿠폰 칸」 택1** (2026-07-23 라이브 실측 · 사장님 질문에서 발견): PDP 「쿠폰받기」 레이어(`div.layer_down_coupon .coupon_list li` → `.name`·`.price`/`.per`)에 있고, **확장이 이미 받는 원본 SSR HTML(403KB) 안**이라 **추가 API 호출 0**(`crawlers/lotteon.py::_parse_download_coupons`).
  - 🔴 **무조건 차감 금지** — 아이몰 쿠폰함 공식 문구 「상품별로 **할인쿠폰/카드할인/TV쇼핑할인 중 1개만 선택**」. 표면가엔 이미 「쿠폰할인」이 들어가 있어(`dataBenefit.fullDiscountObj.discountList`) 덧붙이면 이중차감 = 매입가 과소 = 마진 착시. 계산은 **정가 기준 택1 비교**(`resolve_download_coupon_saving`): `정가 = 표면가 + 선반영쿠폰` → `대안가 = 정가 − 쿠폰` → `추가차감 = max(0, 표면가 − 대안가)`. 쿠폰 여러 장이어도 **1장만**.
  - 실측 검산(goods_no=2559138690, 르무통 메이트): 표면 119,900 · 선반영 쿠폰 29,100 · 다운로드 5% → 정가 149,000 · 대안가 141,550 > 119,900 → **추가차감 0**(5%로 바꾸면 21,650원 더 비쌈). 최종매입가 113,300 유지.
...
| P31 | 롯데아이몰 | 백엔드 | ✅ | PDP 「쿠폰받기」의 다운로드 쿠폰(예: [르무통] 5%)이 계산식에 전혀 안 뜸 — 사장님이 화면 보고 지적 | 아이몰 크롤러가 표면가·L.POINT·자동 카드할인 3개만 뽑고 **쿠폰 파서가 아예 없었다**(파일 전체 쿠폰 문자열 0건). 값이 틀린 게 아니라 **입력이 안 들어간 것** | 원본 SSR HTML 에서 수집(`_parse_download_coupons` — 추가 API 0). ⚠️**차감은 택1** — 아이몰 공식 「할인쿠폰/카드할인/TV쇼핑할인 중 1개」라 선반영 「쿠폰할인」과 정가 기준 비교해 **차액만**(`resolve_download_coupon_saving`). 실측 상품은 선반영 29,100 ≫ 5% 7,450 이라 **추가차감 0 이 정답** — 「쿠폰이 있으니 깎자」로 가면 매입가 과소(P30 과 같은 사고 클래스) | `lemouton/sourcing/crawlers/lotteon.py::resolve_download_coupon_saving` |
...
> **빠른 복귀 인덱스** — 혜택 안 뜸→P3 / 가짜 '재고있음'→S1·S2·S3·S8(`_resolve_stock`·중복행) / 셀≠계산식→P4 / 다운로드 쿠폰 안 깎임→P31(택1이라 정상일 수 있음) / 계산식(fx)이 안 열림·500→P28·P29(카탈로그 소싱처는 source_id 가 문자키) / 경유·캐시백 둘 다 깎임→P30 / 신규 URL '미실시'→P6 / 크롤 후 매트릭스 stale→`window.loadMatrix`·`SM_BREAKDOWNS` / dev 값 이상→O4 / 표면가≠매입가 혼동→§3(셀 큰 숫자=최종매입가).
