...
| P15 | 가격설정 일괄저장 | 백엔드 | ⏳ | 허용목록 밖 필드를 조용히 무시 | `api_pricing.py` `price-config/bulk` 허용목록 밖 무음 skip | 거부 필드 보고 | — |

### 🛠️ 기타 (10)  · O7~O10 = 저장(save) 단계
...
> **알려진 구조적 결함(보충)**: SSG/SSF 혜택이 라이브 확장크롤로 갱신 안 됨(P3) — `toItemBG`(background.js)·`save_crawl_result`(api_pricing.py:1115)가 **price·stock만 저장**하고 `dynamic_benefits_json`(ssg_money 등) 미저장. + 서버 단일 재크롤 `refetch`도 `_detect_site_from_url`이 ssg.com→'ssf' 오분류·크롤러 dict에 'ssg' 키 없음.
...
*최종 수정: 2026-06-27 §5를 '에러 이력 카탈로그'(재고12·가격15·기타10 = 37건, 2축 분류 + 상태 + 커밋, 조용한 실패 3원칙)로 전면 교체 — 기존 '재발' 복귀표 흡수. 상단 안내문 추가. / 2026-06-26 §8 동기화 규칙(코드↔원문·보기, 3주체 순서·자동/절차) 신설. + §7 신규 소싱처 온보딩 배선 체크리스트(B1~B9·E1~E4·D1~D2·경로 A/B). 코드 변경 시 해당 `💻`·배선 라인 동기화. 소싱처 추가 시 §1·§2 표 행 + §6 형 분류 + §7 자가검증 + §8 절차.*
