...
- **무신사** (`compute_breakdown` 의 `if str(source_id) == '3':` 분기 — `api_benefits.py:970~1009` 힌트): **표면 노출가**(surface_price = `goodsPrice.salePrice`, 사이트 노출가. **회원가 아님** — 회원가는 로그인 전용 별도 필드 `member_price` 이고 값이 다르다, 예: 표면가 119,900 ≠ 회원가 101,510) → 차감은 정액(원) 먼저·정률(%) 나중(`final_price.py:_benefit_priority`, 2026-07-19부터). 정액 그룹 안에서는 삽입 순서 그대로: −후기적립(500원 고정, 템플릿 혜택이라 가장 먼저 병합됨) → −상품쿠폰 → −등급할인 → −등급적립 → −무신사머니 결제적립 (라이브 실측 순서, 2026-07-20) → 결제 택1(정률): 무신사머니 결제적립이 활성(금액 > 0)이면 그걸로 끝나고 현대카드는 비활성, 없으면 현대카드 2.73% 청구할인 fallback. ⚠️ 무신사머니는 **금액**이지 %가 아니다 — 코드 어디에도 "3%" 리터럴은 없다(잘못된 옛 기술이었음).
...
| P9 | 무신사 등 | 백엔드 | ✅ | 혜택 금액 0으로 저장(부분 스냅샷이 템플릿 가림) | 부분 스냅샷 override가 템플릿 전체 가림은 이름 기준 병합으로 해소(P20과 동일 수정). 무신사 비로그인 금액0 clear는 의도(폴백 금지, 버그 아님) | 이름 기준 병합(`compute_breakdown` 의 `effective = []` 조립 루프 — `api_benefits.py:718~729` 힌트) | 2026-06-22 |
...
| P24 | 무신사 등 | 백엔드 | ✅ | 모음전 1건·상품 7개에서 옵션 65%가 **남의 상품** 가격·쿠폰으로 계산(매입가 과소) | `compute_breakdown` 가드 `if source_product_id and not _dynamic_benefits` — 낡은 OptionSourceUrl 조회가 먼저 뭔가 찾으면(무신사는 항상) 매트릭스가 넘긴 정답 `source_product_id` 를 무시 | 가드를 `if source_product_id` 로 — 명시 지정이 낡은 조회를 무조건 이김(site·deleted_at 검증 후 동적혜택 교체) | eb0aca57 |
| P25 | 무신사 | 백엔드 | ✅ | 저장경로 2곳 코드 어긋남 — 빈 상품쿠폰이 와도 한쪽만 옛 쿠폰 pop(없어진 쿠폰이 계속 차감될 위험) | `api_pricing.py` 저장 블록엔 else(pop) 있는데 `sources/service.py::save_crawl_result` 만 누락 — 현재는 전체 덮어쓰기라 결과 동일하지만 병합 리팩터 시 조용히 갈라짐 | 두 경로 코드 대칭 통일 + 특성 박제 테스트(test_musinsa_coupon_clear) | 6e263796 |
| P26 | 전 소싱처 | 백엔드 | ✅ | 혜택 이름 끝 띄어쓰기만 달라도 게이트가 조용히 통과(중복/누락 판정 오류) | 혜택 이름 비교가 raw 문자열 — 공백 변형이 다른 이름으로 취급 | 이름 정규화(strip) 후 비교 | e7ac72ac |
...
| O21 | 확장 | 프론트(확장) | ✅ | 화면이 확장 버전을 실제보다 낮게 판단(구버전 안내가 엉뚱한 숫자) | `content_mou.js` 의 `MOUM_EXT_VERSION`(data-moum-ext 원천)이 manifest 와 어긋남 — 버전 상수가 3곳(manifest·background·content) | 3곳을 항상 같이 올린다(주석 박제) — 릴리스 시 3곳 일괄 확인 | 7ad2643f |
...
| B8 | 소싱처별 혜택 차감 블록 | `api_benefits.py:406` `compute_breakdown` (소싱처 분기는 `source_ids.site_key()` 호출 이후) | 혜택이 매입가에서 안 빠짐 |
