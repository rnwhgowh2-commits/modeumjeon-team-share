...
| S14 | 전 소싱처 | 기타(엔진) | ✅ⓑ / ⏳ⓐ | 전체크롤 미완료 시 전 매트릭스가 가짜 '재고있음' + 유효가격 없는 옵션 판매차단 안 됨 | **2026-06-28 정밀 추적**: ⓐ `_resolve_stock`(`api_pricing.py:216`)이 `raw is None→'재고있음'`(가이드 §1 'None=미크롤'과 상충) ⓑ **판매차단 게이트 inert** — 하드리셋·finalize 함수(`_reset_bundle_crawl_state`·`_finalize_bundle_crawl_block`)와 `crawl_blocked` 컬럼이 **2026-06-22 stale 브랜치 머지(94466889, `feature/crawl-worker-queue`)에서 동반 유실**. `service.py:441·470`의 `try/except: pass`가 ImportError를 조용히 삼켜 둘 다 **no-op**였음 → `preview.py:149` 게이트 영구 inert | **ⓑ 2026-06-28 복원 완료** — `crawl_blocked` 컬럼(`models.py`+`shared/db.py` ensure-columns, Postgres·SQLite 양쪽) + `_reset_bundle_crawl_state`·`_finalize_bundle_crawl_block`·`_sources_have_valid_price`(`api_pricing.py`) 복원, service.py import 정상화→하드리셋·finalize 재가동. 통합테스트 3건 PASS(`test_crawl_reset_finalize_integration`). **ⓐ None=미크롤 시맨틱은 광범위영향 → 별도**. ✅라이브 배포완료(미존재 라우트=404 vs 복원 엔드포인트=401로 코드 라이브 확정). 실크롤 행동검증만 대기 | 8cc2faa4 |
