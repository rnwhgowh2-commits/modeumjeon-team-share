...
🛡️ **공통 근본 패턴 — "조용한 실패(silent failure)"**: ① 실패를 삼키고 성공처럼 보임 ② delete-then-insert가 빈/부분 페이로드로 데이터 삭제 ③ 무제한 동시성이 인프라 한도 초과. → **재발 방지 3원칙: (a) 저장 실패는 항상 사용자에게 표면화 · (b) 삭제형 저장은 빈 페이로드 가드 · (c) 대량 저장은 동시성 제한 + 멱등 재시도.** (§4 무결성 4원칙의 저장판)

### 🛡️ 원칙 ← 출처 에러 (§4 무결성 4원칙은 모두 실제 에러에서 나왔다)

> 원칙은 추상이 아니라 **사고의 응결**이다. 원칙을 어기면 아래 에러가 그대로 재발한다. (§4 ↔ 이 카탈로그 양방향 추적)

| §4 원칙 | 이 원칙을 낳은 에러 |
|---|---|
| **① 폴백가 금지** | P23(가짜 95000) · S1·S2·S8(상품합계 재고 폴백 둔갑) · [[feedback_no_fallback_price_on_match_fail]] |
| **② 크롤실패=표기** | S14(하드리셋·판매차단 — 현재 inert) · P7(일시실패 stale 정체) · P6(신규URL 조용히 드롭) |
...
| **조용한 실패 3원칙** (저장판) | (a)표면화 ← O7·O9·O10·O5 / (b)빈 페이로드 가드 ← P11 / (c)동시성 제한 ← P10 |
...
| S14 | 전 소싱처 | 기타(엔진) | ⏳ | 전체크롤 미완료 시 전 매트릭스가 가짜 '재고있음' + 유효가격 없는 옵션 판매차단 안 됨 | **2026-06-28 정밀 추적**: ⓐ `_resolve_stock`(`api_pricing.py:216`)이 `raw is None→'재고있음'`(가이드 §1 'None=미크롤'과 상충) ⓑ **판매차단 게이트 inert** — 하드리셋·finalize 함수(`_reset_bundle_crawl_state`·`_finalize_bundle_crawl_block`, 7da62bde 추가)가 **제거됨**(`crawl_blocked` 컬럼도 모델에서 사라짐). `service.py:441·470`의 `try/except: pass`가 ImportError를 조용히 삼켜 둘 다 **no-op** → `preview.py:149`가 읽는 `crawl_blocked`가 영영 안 켜짐(판매차단 무력) | ⚠️ **소유자 결정 필요** — 현재 모델(per-option `current_stock` + NULL보존)에 판매차단 재구현 vs 의도적 제거 확정 후 dead import·게이트 정리. + None=미크롤 분기. (별도 조사 task) | — |
...
| S16 | 무신사 | 프론트(확장) | ⏳ | 인벤토리 API 호출 통째 실패 시 전 옵션 999 '재고있음' 둔갑 | 확장 추출기가 에러 swallow→`invMap` 빈 상태에서 `remainQuantity==null ? 999` (호출실패와 미매칭 미구분) | API 전체 실패 시 999 금지→불명(-1). §1 함정 규칙을 코드가 위반 중 | — |
...
| P3 | SSG·SSF | 백엔드 | ⏳ | SSG MONEY 적립이 라이브크롤로 영원히 안 채워짐 | `toItemBG`·`save_crawl_result` 둘 다 혜택필드 누락(price/stock만) | 확장크롤 혜택 저장 배선 | — |
...
| P9 | 무신사 등 | 백엔드 | ✅ | 혜택 금액 0으로 저장(부분 스냅샷이 템플릿 가림) | 부분 스냅샷 override가 템플릿 전체 가림은 이름 기준 병합으로 해소(P20과 동일 수정). 무신사 비로그인 금액0 clear는 의도(폴백 금지, 버그 아님) | 이름 기준 병합(`api_benefits.py:572`) | 2026-06-22 |
...
| P12 | 옵션조합 모달 | 프론트 | ⏳ | 축 값 이름변경(240→245) 시 그 옵션 매핑이 조용히 유실 | rename 시 매핑 키 마이그레이션 없음 | rename 시 매핑 키 마이그레이션 | — |
...
### 🛠️ 기타 (16)  · O7~O10 = 저장(save) 단계

| # | 영역 | 계층 | 상태 | 증상 | 근본원인 | 재발방지 체크 | 커밋 |
|---|---|---|---|---|---|---|---|
| O1 | 구조(원칙) | 기타 | ⏳ | 조용한 실패 버그 클래스(메이트 중복·표면가 불일치·SSG 완료 위장) | 무결성 제약 부재(A) + SSOT 분열(last_price 평균 vs 최저)(B) + 누락 무경고(None 탈락)(C) | DB 제약 + SSOT 통일 + None 후보 경고 (상단 3원칙) | — |
| O2 | 배포 | 기타(인프라) | ✅ | 배포 success인데 옛 코드 영속 | `docker run`이 기존 컨테이너 미교체로 백엔드 옛 코드로 계속 돎 | `docker rm -f` + 교체 검증(success ≠ 코드 라이브) | — |
| O3 | 확장 | 기타 | ⏳주의 | 수정해도 라이브 미반영 | 확장 버전 분열 + 로드된 확장 = 데스크톱 사본(dev 사본 고쳐도 무효) | 로드 확장 경로 확인 후 reload | — |
| O4 | DB | 기타(데이터) | ⏳주의 | dev 값이 라이브와 다름 | dev DB는 라이브보다 수십~수백 커밋 stale | 라이브 대조 필수(dev 신뢰 금지) | — |
...
| O6 | 롯데온 per-size | 기타(미구현) | ⏳ | 롯데온은 사이즈별 수량 아닌 상품단위 999/0만 들어옴 | 확장 `lotteonExtractor`가 per-option 재고 미수집(상품 DOM만) — '저장 누락'과 달리 '수집 자체 미구현' | per-size 수집 extractor 구현(별도 작업) | — |
...
| O13 | 서버 parse | 백엔드 | ⏳ | 전체크롤 일부 건 SyntaxError(`<!DOCTYPE` 응답) | `/api/sources/parse`가 JSON 대신 HTML 에러페이지를 간헐 반환→클라 `r.json()` throw | 응답 content-type·본문 검증 후 명확 처리 | — |
| O14 | 전 소싱처 | 프론트(확장) | ⏳ | 일부 등록 URL이 전체크롤 후에도 pending(아예 안 긁힘), 단건 recrawl-url로는 정상 | 확장 전체크롤 오케스트레이션이 일부 URL을 enqueue에서 누락(coverage gap, 미확정) | enqueue 커버리지 점검·전 URL 포함 보장 | — |
...
> **알려진 구조적 결함(보충)**: SSG/SSF 혜택이 라이브 확장크롤로 갱신 안 됨(P3) — `toItemBG`·`save_crawl_result`(api_pricing.py:1115)가 **price·stock만 저장**하고 `dynamic_benefits_json`(ssg_money 등) 미저장. (refetch의 SSG→SSF 오분류는 O16에서 `ssg.com→'ssg'`로 수정됨, 2026-06-28.)

> **빠른 복귀 인덱스** — 혜택 안 뜸→P3 / 가짜 '재고있음'→S1·S2·S3·S8(`_resolve_stock`·중복행) / 셀≠계산식→P4 / 신규 URL '미실시'→P6 / 크롤 후 매트릭스 stale→`window.loadMatrix`·`SM_BREAKDOWNS` / dev 값 이상→O4 / 표면가≠매입가 혼동→§3(셀 큰 숫자=최종매입가).

> **2026-06-28 추적·수정** — ⏳ 17건 현행 코드 추적 → **6건 수정**(O16·P15·P13·P23·O5 + P9 기수정 확인 → ✅). 2차 추적에서 잔여 11건은 대부분 **repo 단독 수정 불가**로 확인: ① **라이브 데스크톱 확장(v0.7.x) 소관** — P3(혜택 전송)·S16(무신사 999)·O14(enqueue 커버리지)·O6(per-size 미구현)·O13(parse 호출도 확장 측). repo 확장은 stale(v0.4.3)이라 라이브 미반영 → 확장 배포 필요. ② **소유자 설계 결정 필요** — **S14**: 하드리셋·판매차단 함수(7da62bde)가 제거되고 `crawl_blocked` 컬럼도 사라져 `preview.py:149` 게이트가 inert(`service.py`의 `try/except: pass`가 ImportError 조용히 삼킴). 재구현 vs 제거확정은 owner 결정 / **None=재고있음**(`_resolve_stock:216`) 시맨틱 변경도 광범위 영향이라 일방 금지. ③ **fragile/app-전역** — P12(매핑 모달 rename 흐름 깊은 변경)·O13 서버측(글로벌 에러핸들러 app 전역). ④ **코드 단건 대상 아님** — P14(준비중 표시 완료, 실제 재계산은 feature)·O1(원칙=상단 3원칙)·O3·O4(운영 주의). + **방어 가드 추가**(2026-06-28): `ext_bridge.js` `fetchJson`로 비-JSON 응답 시 조용한 SyntaxError 차단(option-matrix·codes). S14 reset/finalize 정의누락은 별도 조사 task 위임.
