...
- **SSG** (`api_benefits.py:570~674·764`): 최적가(bestAmt, 즉시할인 선반영) → −SSG MONEY 적립(①무조건=항상 / ②충전결제=rate≥3%만) → −카드혜택가 → −상품쿠폰 → −리뷰적립(텍스트) 50원(시드) − OK캐시백 2%(시드, `base_ratio` **1.0** 전액 예외 — 카드와 동시 차감) / 현대카드 2.73% 청구할인 fallback(네이버페이 제외)
...
|---|---|---|
| **Python 서버 크롤** (fetch_one_source/배치) | `upsert_source_product` — 키 화이트리스트 단일 원천 `OPTION_DYNAMIC_KEYS`/`PRODUCT_DYNAMIC_KEYS` (`lemouton/sources/service.py:606~638` 힌트, 롯데온 3키 포함) | ✅ **저장됨** |
