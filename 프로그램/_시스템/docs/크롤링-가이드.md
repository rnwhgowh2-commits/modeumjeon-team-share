...
> **2026-06-28 추적·수정** — ⏳ 17건을 현행 코드로 추적해 **6건 수정**(O16·P15·P13·P23·O5 + P9 기수정 확인 → ✅). **잔여 ⏳ 사유**: P3·S16·O14 = 라이브 데스크톱 확장(v0.7.x) 소관이라 repo로 못 고침(확장 배포 필요) / P12·S14 = money-critical 매핑·재고 핵심 로직이라 깊은 변경 보류(⚠️ S14 조사 중 `_reset_bundle_crawl_state`·`_finalize_bundle_crawl_block`가 `service.py`서 import되나 `api_pricing.py`에 **정의 누락** 발견 → 하드리셋·판매차단 잠재 미작동, 별도 조사) / O13 = 클라 가드는 가능하나 서버 글로벌 에러핸들러가 app 전역이라 별도 검토 / P14 = '준비중' 표시로 오인만 차단(실제 재계산 미구현) / O1·O3·O4·O6 = 구조 원칙·운영 주의·미구현 feature(코드 단건 수정 대상 아님).
