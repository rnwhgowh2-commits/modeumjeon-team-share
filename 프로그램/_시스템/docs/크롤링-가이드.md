...
### ✅ 마지막 — 라이브 자가검증 6종 (하나라도 빠지면 위 표 역추적)
...
| 어긋남 | 자동 감지 |
|---|---|
| 신규 소싱처가 가이드에 빠짐 | ✅ (예정 — P1) |
| 확장 버전이 가이드 기준과 다름 (전체크롤 방식 바뀜) | ✅ (예정 — P3·C1) |
| 인용 심볼 사라짐·이름변경 | ✅ (예정 — P3·C2) |
| 순수 로직만 조용히 바뀜(버전·심볼 그대로) | ❌ → 코드 고칠 때 가이드도 같이(절차) |

> ⚠️ **현재 상태**: 4·5의 **자동 배너는 구현 예정**(스펙 `docs/superpowers/specs/2026-06-26-크롤가이드-자동동기화-정합성가드-design.md`). **지금은 절차로 운영** — Claude가 코드 고칠 때 원문+보기를 같이 갱신한다. 배너가 붙으면 위 표의 ✅(예정)을 (적용)으로 바꾼다.

*최종 수정: 2026-06-26 §8 동기화 규칙(코드↔원문·보기, 3주체 순서·자동/절차) 신설. + §7 신규 소싱처 온보딩 배선 체크리스트(B1~B9·E1~E4·D1~D2·경로 A/B). 코드 변경 시 해당 `💻`·배선 라인 동기화. 소싱처 추가 시 §1·§2 표 행 + §6 형 분류 + §7 자가검증 + §8 절차.*
