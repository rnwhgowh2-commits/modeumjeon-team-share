...
| O18 | 매트릭스/위젯 | 백엔드+프(확장) | ✅셀·매트릭스보기 / ⏳위젯 | 커스텀 소싱처(hmall·롯데아이몰)가 **정상 크롤됐는데** 셀·매트릭스보기·위젯에 안 뜸 | 매트릭스가 소싱처를 `source_id`(레지스트리 id)로 컬럼·셀 매칭 → 커스텀은 `_key_domain`(builtin 6 하드코딩) 밖이라 셀 `source_id=null` → `deriveSourceColumns`가 null 컬럼 skip. 위젯 카운트도 builtin 기준 | **셀·매트릭스보기 ✅(9db59f6f)**: option-matrix 셀 `source_id='key:'+source_key` 합성 + `DATA.sources` 커스텀 컬럼 추가 + `get_labels`에 `get_all_sources`(SourcingSource) 한글 라벨. `bulk_breakdowns` int 실패 try/except 흡수 → 커스텀 셀 표면가 폴백. ⏳위젯 카운트 확장측 별도 | 9db59f6f |
