...
> 2026-07-23 신설(M4-4). 마켓 등록의 필수 입력값이다 — **6마켓 전부 대표 이미지 필수**,
...
| **SSG** | JSON-LD `Product.image` **8장**(`_i{1..8}_1200.jpg`) · 폴백 `og:image`(250px) | ❌ **확인불가** — 교차출처 iframe(`itemdesc.ssg.com/item/iframePItemDtlDesc.ssg?itemId=…`) 안에 있어 페이지 HTML·렌더 outerHTML 어디에도 없다 | `crawlers/ssg.py::_parse_image_urls`·`_parse_detail_html`(사유 docstring) |
| **무신사·롯데온** | ⏳ 미착수 — 추출이 확장 `background.js` 라 서버 배포만으론 안 바뀐다 | ⏳ 미착수 | 확장 |
| **현대H몰·롯데아이몰·스스르무통** | ⏳ 미착수 | ⏳ 미착수 | — |
...
- ⚠️ **상세는 '상품 상세'만 좁혀 담는다** — 르무통 `#proDetail` 통째 = 쇼핑몰 시즌 이벤트 배너(남의 몰 홍보), SSF `#godsDetailInfoTab` 통째 = 추천상품 36장 + SSF 내부 상품번호. 좁힌 컨테이너만 쓴다.
...
**정리 규칙(공통)** — `base.build_image_urls`(절대화·순서유지 중복제거·아이콘/로고/1px/스킨호스트 배제·확장자 확증·0장이면 경고) · `base.sanitize_detail_html`(script·iframe·media·svg 제거 / **`a`·`picture` unwrap** / on* 핸들러·HTML 주석 제거 / img 절대화·비상품 img 제거 / 알맹이(=주소 있는 img 또는 텍스트) 없으면 빈 문자열 / 태그 경계 컷).
...
동일성 키 = `product_drafts.source_url`(정규화형만 저장).
...
- 🔴 **매입가를 판매가로 쓰지 않는다** — `last_price`·`current_price` 는 **우리가 사는 값**이다.
  ...
  「0원에 팔겠다」가 아니다.
- 🔴 **옵션 추가금도 0** — 옵션별 매입가 차이는 **판매 정책이 아니다**. 대신 매입가가 갈리면
  경고를 띄운다(그대로 두면 비싼 옵션이 싼 값에 팔린다 = 손해).
...
- ⚠️ **초안은 URL 당 1벌** — 이미 있으면 새로 만들지 않고 **크롤 소유 칸만** 갱신한다
  (옵션·재고·이미지·상세·카테고리경로). 판매가·고시·A/S·배송비·매입가마진 6칸은 사람이 채운
  값이라 손대지 않고, 이름·브랜드는 **비어 있을 때만** 채운다. 이미 등록됨/등록중이면 아예
  덮지 않는다(마켓에 올라간 내용과 갈리면 어느 쪽이 진짜인지 모르게 된다).
  지운 초안(`deleted_at`)은 되살리지 않고 새로 만든다.

**여전히 사람 몫(크롤이 원리상 못 주는 것)** — 판매가 · 상품고시정보 · A/S 전화·안내 ·
배송비·반품비 · 고시유형(기본 「의류」) · 브랜드(`SourceProduct` 에 컬럼이 없다) ·
마켓 카테고리 코드. 고시 13~14칸은 「설정 > 고시정보 기본값」이 채워 두면 사전 점검이 자동으로 쓴다.
...
> **빠른 복귀 인덱스** — 혜택 안 뜸→P3 / 가짜 '재고있음'→S1·S2·S3·S8(`_resolve_stock`·중복행) / 셀≠계산식→P4 / 다운로드 쿠폰 안 깎임→P31(플러스 칸 — 경유 쿠폰과 택1) / 계산식(fx)이 안 열림·500→P28·P29(카탈로그 소싱처는 source_id 가 문자키) / 경유·캐시백 둘 다 깎임→P30 / 신규 URL '미실시'→P6 / 크롤 후 매트릭스 stale→`window.loadMatrix`·`SM_BREAKDOWNS` / dev 값 이상→O4 / 표면가≠매입가 혼동→§3(셀 큰 숫자=최종매입가).
