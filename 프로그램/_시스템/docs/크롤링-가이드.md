...
| S18 | 업로드 변동감지 | 백엔드 | ⏳ | 한정 1개 재고를 '사실상 품절'로 분류 가능 | `change_detection.py` `STOCK_AVAILABLE_THRESHOLD=2` → `stock==1`을 '사실상 품절' 그룹으로 → PUT(마켓 푸시) skip 판단에서 한정 1개의 restock/soldout 전이를 못 잡는 경계(주석은 raw_stock 그대로 전송이라 방어 주장) | 임계치 재검토 또는 raw_stock 기준 전이감지 (⚠️설계 의도일 수 있어 확인 필요) | — |
...
### 🛠️ 기타 (17)  · O7~O10 = 저장(save) 단계

| # | 영역 | 계층 | 상태 | 증상 | 근본원인 | 재발방지 체크 | 커밋 |
|---|---|---|---|---|---|---|---|
| O1 | 구조(철칙) | 기타 | ⏳ | 조용한 실패 버그 클래스(메이트 중복·표면가 불일치·SSG 완료 위장) | 무결성 제약 부재(A) + SSOT 분열(last_price 평균 vs 최저)(B) + 누락 무경고(None 탈락)(C) | DB 제약 + SSOT 통일 + None 후보 경고 (상단 3철칙) | — |
